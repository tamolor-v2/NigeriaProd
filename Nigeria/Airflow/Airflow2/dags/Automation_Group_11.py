...
d_1 = Variable.get("rerunDate11", deserialize_json=True)
...
dag = DAG('New_Automation_Group_11', default_args=default_args, catchup=False,schedule_interval='15 5 * * *')

ValidationCode = 'python3.6 /nas/share05/tools/ValidationTool_Python/bin/validationTool.py -d %s -f group11 -c config.json' %(d_1)
...
feedname_QRIOS_TRANSACTIONS = 'QRIOS_TRANSACTIONS'.upper()
feedname2_QRIOS_TRANSACTIONS = 'QRIOS_TRANSACTIONS'.lower()

dedup_command_QRIOS_TRANSACTIONS="bash /nas/share05/tools/DQ/Dedup.sh %s %s " % (d_1,feedname_QRIOS_TRANSACTIONS)
logging.info(dedup_command_QRIOS_TRANSACTIONS) 

pcf_command_QRIOS_TRANSACTIONS = "java -Dlog4j2.formatMsgNoLookups=true -Dlog4j.configurationFile=/nas/share05/tools/Reprocessing/conf/log4j2.xml  -cp /nas/share05/tools/Reprocessing/lib/ReprocessingTool-1.0.7.jar:/nas/share05/tools/Reprocessing/lib/FileOps-1.0.1.190614.jar:/usr/hdp/current/hadoop-client/conf com.ligadata.reprocessing.App --cfgPath /nas/share05/tools/Reprocessing/conf/config_new_presto_FDI_local_edge2.json -mfq /nas/share05/tools/Reprocessing/conf/MissingFilesDetectionQuery.sql -wd /tmp/ -sd %s -ed %s -f %s --logQueries --ignoreDotFilesCheck -p 2 --move " % (d_1,d_1,feedname_QRIOS_TRANSACTIONS)
logging.info(pcf_command_QRIOS_TRANSACTIONS)

PCFCheck_command_QRIOS_TRANSACTIONS = "hive --outputformat=tsv2 --showHeader=false -e 'MSCK REPAIR TABLE audit.reprocessing_summary_report;' && bash /nas/share05/tools/DQ/CheckPCFTable.scala -feedname %s -date %s -sleeptime %s" % (feedname_QRIOS_TRANSACTIONS,run_date,sleeptime)
logging.info(PCFCheck_command_QRIOS_TRANSACTIONS)

branchScript_QRIOS_TRANSACTIONS = "bash /nas/share05/tools/DQ/CheckValidationTable.scala -feedname %s -date %s -sleeptime %s -retry 200 -rundate %s -hour %s -outPath %s " %(feedname_QRIOS_TRANSACTIONS,d_1,sleeptime,run_date,hour_run,pathcsv)
logging.info(branchScript_QRIOS_TRANSACTIONS)

def branch_QRIOS_TRANSACTIONS():
    logging.info(os.getenv('HOSTNAME'))
    filepath = '%s/%s/%s/%s' % (pathcsv,d_1,feedname2_QRIOS_TRANSACTIONS,feedname2_QRIOS_TRANSACTIONS)
    if os.path.isfile(filepath):
        os.remove(filepath)
        logging.info('remove file %s'%(filepath))
        
    os.system(branchScript_QRIOS_TRANSACTIONS)
    x = readcsv(filepath,feedname2_QRIOS_TRANSACTIONS)
    logging.info('branch_QRIOS_TRANSACTIONS' + x)
    return x

Branching_QRIOS_TRANSACTIONS = BranchPythonOperator(
    task_id='branchid_QRIOS_TRANSACTIONS',
    python_callable=branch_QRIOS_TRANSACTIONS,
    dag=dag,
    run_as_user='daasuser',
    priority_weight=1
)

PCF_QRIOS_TRANSACTIONS = BashOperator(
    task_id='PCF_QRIOS_TRANSACTIONS',
    bash_command= pcf_command_QRIOS_TRANSACTIONS,
    dag=dag,
    queue='edge01002',
    run_as_user='daasuser',
    priority_weight=1
...
Dedup_QRIOS_TRANSACTIONS = BashOperator(
    task_id='Dedup_QRIOS_TRANSACTIONS',
    bash_command= dedup_command_QRIOS_TRANSACTIONS,
    dag=dag,
    queue='edge01002',
    run_as_user='daasuser',
    priority_weight=1
)
Dedup2_QRIOS_TRANSACTIONS = BashOperator(
    task_id='Dedup2_QRIOS_TRANSACTIONS',
    bash_command= dedup_command_QRIOS_TRANSACTIONS,
    dag=dag,
    queue='edge01002',
    run_as_user='daasuser',
...
PCFCheck_QRIOS_TRANSACTIONS = BashOperator(
    task_id='PCFCheck_QRIOS_TRANSACTIONS',
    bash_command=PCFCheck_command_QRIOS_TRANSACTIONS,
    dag=dag,
    queue='edge01002',
    run_as_user='daasuser',
    priority_weight=1
...
faile_QRIOS_TRANSACTIONS = EmailOperator(
    task_id='faile_QRIOS_TRANSACTIONS',
    to=email,
    subject='Airflow Alert for feed %s'%(feedname2_QRIOS_TRANSACTIONS),
    html_content=""" <h3>Please check validation</h3> """,
    dag=dag,
    run_as_user='daasuser'
)

delay_python_task_QRIOS_TRANSACTIONS: PythonOperator = PythonOperator(task_id="waitForFlush_QRIOS_TRANSACTIONS",
                                                   dag=dag,
                                                   python_callable=lambda: time.sleep(1800))
            

Branching_QRIOS_TRANSACTIONS >> PCF_QRIOS_TRANSACTIONS >> PCFCheck_QRIOS_TRANSACTIONS >> delay_python_task_QRIOS_TRANSACTIONS >> Dedup_QRIOS_TRANSACTIONS >> Validation_End 
Branching_QRIOS_TRANSACTIONS >> Dedup2_QRIOS_TRANSACTIONS >> Validation_End
Branching_QRIOS_TRANSACTIONS >> faile_QRIOS_TRANSACTIONS
Branching_QRIOS_TRANSACTIONS >> Validation_End


feedname_QRIOS_SDP_TIMEOUT = 'QRIOS_SDP_TIMEOUT'.upper()
feedname2_QRIOS_SDP_TIMEOUT = 'QRIOS_SDP_TIMEOUT'.lower()

dedup_command_QRIOS_SDP_TIMEOUT="bash /nas/share05/tools/DQ/Dedup.sh %s %s " % (d_1,feedname_QRIOS_SDP_TIMEOUT)
logging.info(dedup_command_QRIOS_SDP_TIMEOUT) 

pcf_command_QRIOS_SDP_TIMEOUT = "java -Dlog4j2.formatMsgNoLookups=true -Dlog4j.configurationFile=/nas/share05/tools/Reprocessing/conf/log4j2.xml -cp /nas/share05/tools/Reprocessing/lib/ReprocessingTool-1.0.7.jar:/nas/share05/tools/Reprocessing/lib/FileOps_2.11-0.1-SNAPSHOT_hdfs.jar:/usr/hdp/current/hadoop-client/conf com.ligadata.reprocessing.App --cfgPath /nas/share05/tools/Reprocessing/conf/config_new_presto.json -mfq /nas/share05/tools/Reprocessing/conf/MissingFilesDetectionQuery.sql -wd /tmp/ -sd %s -ed %s -f %s --logQueries --ignoreDotFilesCheck --move  --jdbcName m1004 " % (d_1,d_1,feedname_QRIOS_SDP_TIMEOUT)
logging.info(pcf_command_QRIOS_SDP_TIMEOUT)

PCFCheck_command_QRIOS_SDP_TIMEOUT = "hive --outputformat=tsv2 --showHeader=false -e 'MSCK REPAIR TABLE audit.reprocessing_summary_report;' && bash /nas/share05/tools/DQ/CheckPCFTable.scala -feedname %s -date %s -sleeptime %s" % (feedname_QRIOS_SDP_TIMEOUT,run_date,sleeptime)
logging.info(PCFCheck_command_QRIOS_SDP_TIMEOUT)

branchScript_QRIOS_SDP_TIMEOUT = "bash /nas/share05/tools/DQ/CheckValidationTable.scala -feedname %s -date %s -sleeptime %s -retry 200 -rundate %s -hour %s -outPath %s " %(feedname_QRIOS_SDP_TIMEOUT,d_1,sleeptime,run_date,hour_run,pathcsv)
logging.info(branchScript_QRIOS_SDP_TIMEOUT)

def branch_QRIOS_SDP_TIMEOUT():
    logging.info(os.getenv('HOSTNAME'))
    filepath = '%s/%s/%s/%s' % (pathcsv,d_1,feedname2_QRIOS_SDP_TIMEOUT,feedname2_QRIOS_SDP_TIMEOUT)
    if os.path.isfile(filepath):
        os.remove(filepath)
        logging.info('remove file %s'%(filepath))
        
    os.system(branchScript_QRIOS_SDP_TIMEOUT)
    x = readcsv(filepath,feedname2_QRIOS_SDP_TIMEOUT)
    logging.info('branch_QRIOS_SDP_TIMEOUT' + x)
    return x

Branching_QRIOS_SDP_TIMEOUT = BranchPythonOperator(
    task_id='branchid_QRIOS_SDP_TIMEOUT',
    python_callable=branch_QRIOS_SDP_TIMEOUT,
    dag=dag,
    run_as_user='daasuser',
    priority_weight=1
)

PCF_QRIOS_SDP_TIMEOUT = BashOperator(
    task_id='PCF_QRIOS_SDP_TIMEOUT',
    bash_command= pcf_command_QRIOS_SDP_TIMEOUT,
    dag=dag,
    run_as_user='daasuser',
    priority_weight=1
)

Dedup_QRIOS_SDP_TIMEOUT = BashOperator(
    task_id='Dedup_QRIOS_SDP_TIMEOUT',
    bash_command= dedup_command_QRIOS_SDP_TIMEOUT,
    dag=dag,
    run_as_user='daasuser',
    priority_weight=1
)
Dedup2_QRIOS_SDP_TIMEOUT = BashOperator(
    task_id='Dedup2_QRIOS_SDP_TIMEOUT',
    bash_command= dedup_command_QRIOS_SDP_TIMEOUT,
    dag=dag,
    run_as_user='daasuser',
    priority_weight=1
...
PCFCheck_QRIOS_SDP_TIMEOUT = BashOperator(
    task_id='PCFCheck_QRIOS_SDP_TIMEOUT',
    bash_command=PCFCheck_command_QRIOS_SDP_TIMEOUT,
    dag=dag,
    run_as_user='daasuser',
    priority_weight=1
)

faile_QRIOS_SDP_TIMEOUT = EmailOperator(
    task_id='faile_QRIOS_SDP_TIMEOUT',
    to=email,
    subject='Airflow Alert for feed %s'%(feedname2_QRIOS_SDP_TIMEOUT),
    html_content=""" <h3>Please check validation</h3> """,
    dag=dag,
    run_as_user='daasuser'
)

delay_python_task_QRIOS_SDP_TIMEOUT: PythonOperator = PythonOperator(task_id="waitForFlush_QRIOS_SDP_TIMEOUT",
                                                   dag=dag,
                                                   python_callable=lambda: time.sleep(1800))
            

Branching_QRIOS_SDP_TIMEOUT >> PCF_QRIOS_SDP_TIMEOUT >> PCFCheck_QRIOS_SDP_TIMEOUT >> delay_python_task_QRIOS_SDP_TIMEOUT >> Dedup_QRIOS_SDP_TIMEOUT >> Validation_End 
Branching_QRIOS_SDP_TIMEOUT >> Dedup2_QRIOS_SDP_TIMEOUT >> Validation_End
Branching_QRIOS_SDP_TIMEOUT >> faile_QRIOS_SDP_TIMEOUT
Branching_QRIOS_SDP_TIMEOUT >> Validation_End


feedname_MANUAL_KPI = 'MANUAL_KPI'.upper()
feedname2_MANUAL_KPI = 'MANUAL_KPI'.lower()

dedup_command_MANUAL_KPI="bash /nas/share05/tools/DQ/Dedup.sh %s %s " % (d_1,feedname_MANUAL_KPI)
logging.info(dedup_command_MANUAL_KPI) 

pcf_command_MANUAL_KPI = "java -Dlog4j2.formatMsgNoLookups=true -Dlog4j.configurationFile=/nas/share05/tools/Reprocessing/conf/log4j2.xml -cp /nas/share05/tools/Reprocessing/lib/ReprocessingTool-1.0.7.jar:/nas/share05/tools/Reprocessing/lib/FileOps_2.11-0.1-SNAPSHOT_hdfs.jar:/usr/hdp/current/hadoop-client/conf com.ligadata.reprocessing.App --cfgPath /nas/share05/tools/Reprocessing/conf/config_new_presto.json -mfq /nas/share05/tools/Reprocessing/conf/MissingFilesDetectionQuery.sql -wd /tmp/ -sd %s -ed %s -f %s --logQueries --ignoreDotFilesCheck --move  --jdbcName m1004 " % (d_1,d_1,feedname_MANUAL_KPI)
logging.info(pcf_command_MANUAL_KPI)

PCFCheck_command_MANUAL_KPI = "hive --outputformat=tsv2 --showHeader=false -e 'MSCK REPAIR TABLE audit.reprocessing_summary_report;' && bash /nas/share05/tools/DQ/CheckPCFTable.scala -feedname %s -date %s -sleeptime %s" % (feedname_MANUAL_KPI,run_date,sleeptime)
logging.info(PCFCheck_command_MANUAL_KPI)

branchScript_MANUAL_KPI = "bash /nas/share05/tools/DQ/CheckValidationTable.scala -feedname %s -date %s -sleeptime %s -retry 200 -rundate %s -hour %s -outPath %s " %(feedname_MANUAL_KPI,d_1,sleeptime,run_date,hour_run,pathcsv)
...
delay_python_task_MANUAL_KPI: PythonOperator = PythonOperator(task_id="waitForFlush_MANUAL_KPI",
                                                   dag=dag,
                                                   python_callable=lambda: time.sleep(1800))
            

Branching_MANUAL_KPI >> PCF_MANUAL_KPI >> PCFCheck_MANUAL_KPI >> delay_python_task_MANUAL_KPI >> Dedup_MANUAL_KPI >> Validation_End 
Branching_MANUAL_KPI >> Dedup2_MANUAL_KPI >> Validation_End
Branching_MANUAL_KPI >> faile_MANUAL_KPI
Branching_MANUAL_KPI >> Validation_End
