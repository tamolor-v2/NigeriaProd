...
d_1 = (datetime.now() - timedelta(days=0)).strftime('%Y-%m-%d')
d_2 = (datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d')
d_3 = (datetime.now() - timedelta(days=3)).strftime('%Y-%m-%d')

dag = DAG(
    dag_id='HSDP_SUMMARY',
    default_args=args,
    schedule_interval='* 3,4,5,6,12,18 * * *',
    catchup=False, 
    concurrency=2,
    max_active_runs=1
...
t1 = BashOperator(
     task_id='hsdp_sumd1' ,
     bash_command='/nas/share05/ops/mtnops/NB_summary.py -p HSDP_SUMD -l 2 -s {0} '.format(d_2),
     dag=dag,
...
t2 = BashOperator(
     ...
     bash_command='/nas/share05/ops/mtnops/NB_summary.py -p HYNET_OFFERS -l 2 -s {0} '.format(d_3),
     dag=dag,
     run_as_user = 'daasuser'
)
