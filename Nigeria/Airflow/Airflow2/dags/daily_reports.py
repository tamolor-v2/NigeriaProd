...
date_param1 = (datetime.now() - timedelta(days=0)).strftime('%Y%m%d')
dag = DAG(
...
epostpaid_mou_voice = BashOperator(
     ...
     bash_command='python3.6 /nas/share05/ops/mtnops/daas_epostpaid_mou_detail.py {0} {0} '.format(date_param1),
     dag=dag,
     run_as_user = 'daasuser'
)
...
infraction_late >> SMS_ACTIVATION_REQ >> VisaFone_AOD >> call_to_180 >> CDR_Summary >> cust_ord_invo >> SIM_ACTIVATION >> Epost_Paid_GPRS >> epostpaid_mou_voice>> Hynet_Summary >> ISP_Login >> Momo_Summary >> BANK_TRANSACTIONS >> Neon_Summary 
