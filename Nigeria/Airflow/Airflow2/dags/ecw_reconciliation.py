...
financial_log_recon = BashOperator(
     ...
     bash_command='bash /nas/share05/ops/mtnops/financial_log_recon.sh {0} '.format(d_1),
     dag=dag,
     run_as_user = 'daasuser'
)
...
mfs_audit_log = BashOperator(
     ...
     bash_command='bash /nas/share05/ops/mtnops/mfs_audit_log.sh {0} '.format(d_1),
     dag=dag,
     run_as_user = 'daasuser'
)
...
mfs_transacting_log_recon = BashOperator(
     ...
     bash_command='bash /nas/share05/ops/mtnops/mfs_transacting_log_recon.sh {0} '.format(d_1),
     dag=dag,
     run_as_user = 'daasuser'
)
