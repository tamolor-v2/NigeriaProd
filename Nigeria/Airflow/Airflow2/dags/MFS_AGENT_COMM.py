...
dag = DAG(
    dag_id='MFS_AGENT_COMM',
    default_args=args,
    schedule_interval='0 6 * * *',
    catchup=False,
    max_active_runs=1
)
Insert_MFS_AGENT_COMM = BashOperator(
     task_id='Insert_MFS_AGENT_COMM' ,
     depends_on_past=False,
     bash_command='bash /nas/share05/tools/ExtractTools/MFS_AGENT_COMM.sh `date --date="-1 days" +%Y%m%d`	'  ,
     dag=dag,
