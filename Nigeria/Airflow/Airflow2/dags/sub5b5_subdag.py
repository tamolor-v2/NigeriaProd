...
def getSubDag_sub5b5(parent_dag_name, child_dag_name, args):
    dag = DAG("{0}.{1}".format(parent_dag_name,child_dag_name), default_args=args, catchup=False, max_active_runs=1)


    subbase = BashOperator(task_id='subbase',bash_command='/nas/share05/scripts/segment5b5/run_this.sh {0} {0} segment5b5_subbase daily 9999'.format(date_param_d1) ,      dag=dag,
    ...
    subaggrd = BashOperator(task_id='subaggrd',bash_command='/nas/share05/scripts/segment5b5/run_this.sh {0} {0} segment5b5_subaggrd daily 9999'.format(date_param_d1) ,      dag=dag,
     run_as_user = 'daasuser')
     
    subaggrscd = BashOperator(task_id='subaggrscd',bash_command='/nas/share05/scripts/segment5b5/run_this.sh {0} {0} segment5b5_subaggrscd daily 9999'.format(date_param_d1) ,      dag=dag,
     run_as_user = 'daasuser')
     
    validation_sub = BashOperator(task_id='validation_sub',bash_command='/nas/share05/scripts/segment5b5/run_this.sh {0} {0} segment5b5_validation_sub daily 9999'.format(date_param_d1) ,      dag=dag,
     run_as_user = 'daasuser')
    ...
    dropsubsmartdevnew = BashOperator(task_id='dropsubsmartdevnew',bash_command='bash /nas/share05/scripts/segment5b5/drop_this.sh "nigeria.segment5b5_sub_tmp91"' ,      dag=dag,
     run_as_user = 'daasuser')
    dropsubbase >> subbase >> dropsubgeo >> subgeo >> dropsubusim >> subusim >> dropsubrevrank >> subrevrank >> dropsubrev >> subrev >> dropsubusg  >> subusg >> dropsubsmartdev >> subsmartdev >> dropsubsmartdevnew >> subsmartdevnew >> subfinal >> [subaggrd,subaggrscd] >> validation_sub
