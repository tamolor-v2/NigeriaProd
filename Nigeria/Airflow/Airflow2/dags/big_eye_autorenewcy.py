...
runDt = 20210501
filesToRun = mainTsk(runDt,"be_autorenewcy_")
...
def getSubDag_BigEyeAutorenewcy(parent_dag_name, child_dag_name, args):
    ...
    for i in range(0,len(filesToRun)):
        run_sql_.append(PythonOperator(task_id = 'be_autorenewcy_' + str(i+1),depends_on_past=False,python_callable=excT,op_args=[oPresto,readQ("{sharedPath}/{ftr}".format(sharedPath=sharedPath,ftr=filesToRun[i]))],dag=dag,run_as_user = 'daasuser'))
        if i not in [0]:
