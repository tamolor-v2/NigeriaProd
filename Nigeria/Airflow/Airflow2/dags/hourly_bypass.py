...
hourly_Airtel_bypass = BashOperator(
     ...
     bash_command='bash /nas/share05/ops/mtnops/hourly_Airtel_bypass.sh `date --date="-0 days" +%Y%m%d` ',
     dag=dag,
...
hourly_GLOBACOM_bypass = BashOperator(
     ...
     bash_command='bash /nas/share05/ops/mtnops/hourly_GLOBACOM_bypass.sh `date --date="-0 days" +%Y%m%d` ',
     dag=dag,
     run_as_user = 'daasuser'
)
...
hourly_9mobile_bypass = BashOperator(
     ...
     bash_command='bash /nas/share05/ops/mtnops/hourly_9mobile_bypass.sh `date --date="-0 days" +%Y%m%d` ',
     dag=dag,
     run_as_user = 'daasuser'
)
