...
data = json.loads(open("{0}/config//IFSAPP_CUSTOMER_INFO_TAB_config.json".format(commonPath)).read())
numrun = 0
numhdfs = 0
for x in data['steps']:
...
dag = DAG('Ext_IFSAPP_CUSTOMER_INFO_TAB',
    default_args=args,
...
DeleteOldFiles = BashOperator(
    ...
    bash_command='java -Dlog4j.configurationFile={3}/bin/log4j.xml -cp {3}/lib/ojdbc6.jar:{3}/lib/ExtractionJobsBuilder-1.0.jar  com.ligadata.extractjb.Main -sn delete_old_file -jp {3}/config//IFSAPP_CUSTOMER_INFO_TAB_config.json -d {0} -hr {1} -dr {2} -cp {4}'.format('{{ ti.xcom_pull(key="date" , task_ids="takeDate") }}',runHourStr,dayTodayStr,commonPath,commonPathFeeds),
    dag=dag,
    #provide_context=True,
    run_as_user='daasuser'
) 
...
HDFSCommands = BashOperator(
    ...
    bash_command='java -Dlog4j.configurationFile={3}/bin/log4j.xml -cp {3}/lib/ojdbc6.jar:{3}/lib/ExtractionJobsBuilder-1.0.jar  com.ligadata.extractjb.Main -sn commands -jp {3}/config//IFSAPP_CUSTOMER_INFO_TAB_config.json -d {0} -hr {1} -dr {2}'.format(dateHDFStr,runHourStr,dayTodayStr,commonPath),
    dag=dag,
    #provide_context=True,
    run_as_user='daasuser'
) 
...
alert = EmailOperator(
    task_id='alert',
    to='{0}'.format(emails),
    subject='Airflow IFSAPP_CUSTOMER_INFO_TAB',
    html_content='Finsh extraction IFSAPP_CUSTOMER_INFO_TAB <br>',
    dag=dag,
    ) 
