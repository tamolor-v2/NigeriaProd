...
data = json.loads(open("{0}/config//OTP_STATUS_config.json".format(commonPath)).read())
numrun = 0
numhdfs = 0
for x in data['steps']:
...
CreateDirectory = BashOperator(
    ...
    bash_command='java -Dlog4j.configurationFile={3}/bin/log4j.xml -cp {3}/lib/ojdbc6.jar:{3}/lib/ExtractionJobsBuilder-1.0.jar  com.ligadata.extractjb.Main -sn create_directory -jp {3}/config//OTP_STATUS_config.json -d {0} -hr {1} -dr {2} -cp {4}'.format('{{ ti.xcom_pull(key="date" , task_ids="takeDate") }}',runHourStr,dayTodayStr,commonPath,commonPathFeeds),
    dag=dag,
    provide_context=True,
...
DeleteOldFiles = BashOperator(
    ...
    bash_command='java -Dlog4j.configurationFile={3}/bin/log4j.xml -cp {3}/lib/ojdbc6.jar:{3}/lib/ExtractionJobsBuilder-1.0.jar  com.ligadata.extractjb.Main -sn delete_old_file -jp {3}/config//OTP_STATUS_config.json -d {0} -hr {1} -dr {2} -cp {4}'.format('{{ ti.xcom_pull(key="date" , task_ids="takeDate") }}',runHourStr,dayTodayStr,commonPath,commonPathFeeds),
    dag=dag,
    provide_context=True,
    run_as_user='daasuser'
) 
...
Extract = BashOperator(
    task_id='Extract',
    bash_command='java -Dlog4j.configurationFile={3}/bin/log4j.xml -cp {3}/lib/ojdbc6.jar:{3}/lib/ExtractionJobsBuilder-1.0.jar  com.ligadata.extractjb.Main -sn extract -jp {3}/config//OTP_STATUS_config.json -d {0} -hr {1} -dr {2} -cp {4} {5} '.format('{{ ti.xcom_pull(key="date" , task_ids="takeDate") }}',runHourStr,dayTodayStr,commonPath,commonPathFeeds,psqlURL),
    dag=dag,
    provide_context=True,
    run_as_user='daasuser'
) 
...
MoveFiles = BashOperator(
    task_id='MoveFiles',
    bash_command='java -Dlog4j.configurationFile={3}/bin/log4j.xml -cp {3}/lib/ojdbc6.jar:{3}/lib/ExtractionJobsBuilder-1.0.jar  com.ligadata.extractjb.Main -sn move -jp {3}/config//OTP_STATUS_config.json -d {0} -hr {1} -dr {2} -cp {4} -cs {5}'.format('{{ ti.xcom_pull(key="date" , task_ids="takeDate") }}',runHourStr,dayTodayStr,commonPath,commonPathFeeds,commonSourcePath),
    dag=dag,
    provide_context=True,
    run_as_user='daasuser'
) 
...
alert = EmailOperator(
    task_id='alert',
    to='{0}'.format(emails),
    subject='Airflow OTP_STATUS',
    html_content='Finsh extraction OTP_STATUS <br>',
    dag=dag,
    ) 
