...
data = json.loads(open("{0}/config//NODE_config.json".format(commonPath)).read())
numrun = 0
numhdfs = 0
for x in data['steps']:
...
DeleteOldFiles = BashOperator(
    ...
    bash_command='java -Dlog4j.configurationFile={3}/bin/log4j.xml -cp {3}/lib/ojdbc6.jar:{3}/lib/ExtractionJobsBuilder-1.0.jar  com.ligadata.extractjb.Main -sn delete_old_file -jp {3}/config//NODE_config.json -d {0} -hr {1} -dr {2} -cp {4}'.format('{{ ti.xcom_pull(key="date" , task_ids="takeDate") }}',runHourStr,dayTodayStr,commonPath,commonPathFeeds),
    dag=dag,
    provide_context=True,
    run_as_user='daasuser'
) 
...
ReplaceDumps = BashOperator(
    ...
    bash_command='bash /nas/share05/tools/ExtractTools/scripts/replaceDump.sh NODE {0}'.format(dateRunStr),
    dag=dag,
	run_as_user='daasuser'
	) 
...
alert = EmailOperator(
    task_id='alert',
    to='{0}'.format(emails),
    subject='Airflow NODE',
    html_content='Finsh extraction NODE <br>',
    dag=dag,
    ) 
