...
$check_sql   = "select CAST(date_format(date_add('day', - $ii , current_date ),'%Y%m%d') AS INT) date;";
...
$check_sql   = "select CAST(date_format(date_add('day', - $ii , current_date ),'%d/%m/%Y') AS varchar) date;";
...
$sql_stmt2 ="select ceiling(cast(sum(Recharges) as double)*1.0)  from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
        event_type
from    nigeria.daas_daily_usage_summ_DA
where   date_key =  ".$proc_date."
and   event_type in ('Refill','VTU','DEMAND','DYA')
and network_type='CORE EAST'
group by date_dt,dow,network_type,event_type
order by date_dt 
)
group by region";

$sql_stmt3 ="select ceiling(cast(sum(Recharges) as double)*1.0) from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
$sql_stmt4 ="select ceiling(cast(sum(Recharges) as double)*1.0) from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
where   date_key =  ".$proc_date."
and   event_type in ('Refill','VTU','DEMAND','DYA')
and network_type='CORE WEST'
group by date_dt,dow,network_type,event_type
order by date_dt 
)
group by region";

$sql_stmt5 ="select ceiling(cast(sum(Recharges) as double)*1.0) from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
$sql_stmt6 ="select ceiling(cast(sum(Recharges) as double)*1.0) from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
where   date_key =  ".$proc_date."
and   event_type in ('Refill','VTU','DEMAND','DYA')
and network_type='FAR EAST'
group by date_dt,dow,network_type,event_type
order by date_dt 
)
group by region";

$sql_stmt7 ="select ceiling(cast(sum(Recharges) as double)*1.0) from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
$sql_stmt8 ="select ceiling(cast(sum(Recharges) as double)*1.0) from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
where   date_key =  ".$proc_date."
and   event_type in ('Refill','VTU','DEMAND','DYA')
and network_type='FAR NORTH'
group by date_dt,dow,network_type,event_type
order by date_dt 
)
group by region";

$sql_stmt9 ="select ceiling(cast(sum(Recharges) as double)*1.0) from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
$sql_stmt10 ="select ceiling(cast(sum(Recharges) as double)*1.0) from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
where   date_key =  ".$proc_date."
and   event_type in ('Refill','VTU','DEMAND','DYA')
and network_type='LAGOS/OGUN'
group by date_dt,dow,network_type,event_type
order by date_dt 
)
group by region";

$sql_stmt11 ="select ceiling(cast(sum(Recharges) as double)*1.0) from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
$sql_stmt12 ="select ceiling(cast(sum(Recharges) as double)*1.0) from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
where   date_key =  ".$proc_date."
and   event_type in ('Refill','VTU','DEMAND','DYA')
and network_type='MIDDLE BELT'
group by date_dt,dow,network_type,event_type
order by date_dt 
)
group by region";

$sql_stmt13 ="select ceiling(cast(sum(Recharges) as double)*1.0) from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
$sql_stmt14 ="select ceiling(cast(sum(Recharges) as double)*1.0) from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
where   date_key =  ".$proc_date."
and   event_type in ('Refill','VTU','DEMAND','DYA')
and network_type='UNKNOWN'
group by date_dt,dow,network_type,event_type
order by date_dt 
)
group by region";

$sql_stmt15 ="select ceiling(cast(sum(Recharges) as double)*1.0)  from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
$sql_stmt16 ="select ceiling(cast(sum(Recharges) as double)*1.0) from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
where   date_key =  ".$proc_date."
and   event_type in ('Refill','VTU','DEMAND','DYA')
and network_type='RIVERINE'
group by date_dt,dow,network_type,event_type
order by date_dt 
)
group by region";

$sql_stmt17 ="select ceiling(cast(sum(Recharges) as double)*1.0)  from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
$sql_stmt18 ="select ceiling(cast(sum(Recharges) as double)*1.0) from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
$sql_stmt19 ="select ceiling(cast(sum(Recharges) as double)*1.0)  from (
select 
        concat(date_dt,' ', dow) date,
        sum(amount)              Recharges,
        network_type             Region,
...
@nums = (2347062029498,2347030035237,2349031699027,2348108652827,2348032002722,2347062029762,2347064257509,2348039480583,2348181000016,2349039516051,2347067113221,2348032002141,2348032008175,2347068889651,2347062029700,2348095078301,2348069183948,2347062029888,2349038361230,2348147243462,2348142255283,2348032009259,2348135292397,2348032000244,2348032008647);

#@nums = (2347062029498);   #,2348032004503
...
$riv2 = $riv2*1.0; $riv1  = (substr $riv2, -9, 3).M;
$tot1 = $tot1*1.0;
$tot2 = $tot2*1.0;

$v_msg="GROSS ACTIVATION BY REGION ON $proc_date_day: \nCoreEast: $core_east1  \nCoreWest:$core_west1 \nFarEast: $far_east1 \nFarNorth: $far_north1 \nLagos/Ogun: $lag1 \nMbelt: $mb1 \nUnk: $unk1 \n(Total: $tot1 ) \n(MTD )";
...
print "message: $v_msg\n";
system(`java -classpath "$CLASSPATH:lib/smppapi.jar:lib/commons-logging.jar:lib/log4j-1.2.5.jar:lib/edwsmpp.jar:lib/ojdbc14_g.jar" mtnn.smpp.edw.SyncTransmitter 10.206.140.172 10000 NG_DaaS "7ygv&YGV" "" "$numt" "$v_msg" "DAAS_Alert" 5 9`);
