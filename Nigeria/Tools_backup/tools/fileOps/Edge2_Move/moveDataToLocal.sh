...
PIDFILE=/home/daasuser/PIDFiles/MoveFromlocal_crontab.pid
if [ -f $PIDFILE ]
then
  PID=$(cat $PIDFILE)
...
java -Xmx30g -Xms30g -Dlog4j.configurationFile=/mnt/beegfs/tools/fileOps/log4j2.xml -jar /mnt/beegfs/tools/fileOps/FileOps_2.11-0.1-SNAPSHOT_hdfs.jar -in /data/data_lz/beegfs/live/ -out /mnt/beegfs_bsl/production/movefromlocaltodfs/report/ -mv /mnt/beegfs_bsl/live/ -od -of -tf 1 -op lineCount -nt 64 -dp 15  -rcm -re -nosim -effl "^.*\.(_COPYING_|tmp|TMP)$" -effl "^\..*" 2>&1 | tee "/mnt/beegfs_bsl/production/logs/movefromlocaltodfs_${currDate}_$mytime.log"
