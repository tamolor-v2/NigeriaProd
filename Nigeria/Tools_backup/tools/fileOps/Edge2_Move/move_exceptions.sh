...
PIDFILE=/home/daasuser/PIDFiles/MoveFromlocal_exceptions.pid
if [ -f $PIDFILE ]
then
  PID=$(cat $PIDFILE)
...
#mkdir -p /mnt/beegfs_bsl/production/movefromlocaltodfs/report_new/Exceptions/${currDate}
java -Xmx5g -Xms5g -Dlog4j.configurationFile=/nas/share05/tools/fileOps/log4j2.xml -jar /nas/share05/tools/fileOps/FileOps_2.11-0.1-SNAPSHOT_hdfs.jar -in /data/data_lz/beegfs/live -lbl "exception" -out /nas/share05/production/movefromlocaltodfs/report_new/Exceptions/${currDate}/ -mv /mnt/beegfs_bsl/live/ -od -of -tf 1 -op lineCount -nt 10 -dp 15 -re -rcm -rfl -nosim -effl "^.*\.(_COPYING_|tmp|TMP)$" -effl "^\..*" -idfl "HUAMSC_DaaS|SMSC_TOTAL_DELIVERY_SUCCESS_RATE|SMSC_TOTAL_SUBMIT_SUCCESS_RATE|SMSC_A2P_P2P_P2A_A2A_SUCCESS_RATE|SMSC_LICENSE" 2>&1 | tee "/mnt/beegfs_bsl/production/logs/movefromlocaltodfs_${currDate}_$mytime.log"
