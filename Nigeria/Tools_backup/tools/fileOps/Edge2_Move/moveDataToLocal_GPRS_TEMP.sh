...
PIDFILE=/home/daasuser/PIDFiles/MoveFromlocal_GPRS_TEMP_crontab.pid
if [ -f $PIDFILE ]
then
  PID=$(cat $PIDFILE)
...
mkdir -p /mnt/beegfs_bsl/production/movefromlocaltodfs/report_new/GPRS_MA_TEMP/${currDate}

java -Xmx30g -Xms30g -Dlog4j.configurationFile=/nas/share05/tools/fileOps/log4j2.xml -jar /nas/share05/tools/fileOps/FileOps_2.11-0.1-SNAPSHOT_hdfs.jar -in /data/data_lz/beegfs/live/GPRS_MA_TEMP/incoming -out /mnt/beegfs_bsl/production/movefromlocaltodfs/report_new/GPRS_MA_TEMP/${currDate}/ -mv /data/data_lz/FdiBase/reprocess/CCN_GPRS_MA/ -od -of -tf 1 -op lineCount -nt 30 -dp 15   -re -rcm -rfl -nosim -rg "^([0-9]{8})([0-9]{2}).*_(CCN_GPRS_MA)_.*$" -opp "|@1/|@2" -effl "^.*\.(_COPYING_|tmp|TMP)$" -effl "^\..*" 2>&1 | tee "/mnt/beegfs_bsl/production/logs/movefromlocaltodfs_GPRS_MA_TEMP_${currDate}_$mytime.log"
