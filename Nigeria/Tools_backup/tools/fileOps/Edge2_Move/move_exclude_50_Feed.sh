...
mkdir -p /mnt/beegfs_bsl/production/movefromlocaltodfs_new/report_new/Exclude/${currDate}
java -Xmx30g -Xms30g -Dlog4j.configurationFile=/nas/share05/tools/fileOps/log4j2.xml -jar /nas/share05/tools/fileOps/FileOps_2.11-0.1-SNAPSHOT_hdfs.jar -in /data/data_lz/beegfs/live -out /nas/share05/production/movefromlocaltodfs_new/report_new/Exclude/${currDate}/ -mv /mnt/beegfs_bsl/live/ -od -of -tf 1 -op lineCount -nt 30 -dp 15 -re -rcm -rfl -nosim -effl "^.*\.(_COPYING_|tmp|TMP)$" -effl "^\..*" -edfl "UT_DUMP|SGSN|SDP_ADJ_DA|SDP_DMP_AC|MSC_DaaS|MA_MONITOR|EWP_DYA_ACCOUNT_HOLDER_DUMP|FLYTXT_CAMPAIGN_EVENTS_DAT|SDP_PAM_ALL|SDP_ADJ_AC|CCN_GPRS_AC|CCN_GPRS_DA|CCN_SMS_AC|CCN_SMS_DA|CCN_VOICE_AC|CCN_VOICE_DA|AIR_REFILL_AC|AIR_REFILL_DA|AIR_ADJ_DA|SMSC|CS6_Unified|DPI|MOBILE_MONEY|SDP_ADJ_MA|TMP_DMP_DUMP_ALL|SDP_DMP_MA|UDC_DUMP|SDP_DMP_DA|UC_DUMP|OFFER_DUMP|BUNDLE4U_GPRS|BUNDLE4U_VOICE|CCN_GPRS_MA|CCN_SMS_MA|AIR_ADJ_MA|AIR_REFILL_MA|CCN_VOICE_MA|GGSN|MSC_CDR|NGVS_CDR|GPRS_MA_TEMP|MISSING_FILES_CCN|MVAS_DND_MSISDN_REP_CDR|SMART_APP_DOWNLOAD|HSDP_DOL_LOG|SIS_TOKEN|MSO_MNP_NUMBER_SUMMARY" 2>&1 | tee "/mnt/beegfs_bsl/production/logs/movefromlocaltodfs_${currDate}_$mytime.log"
