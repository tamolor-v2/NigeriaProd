...
PIDFILE=/home/daasuser/PIDFiles/MoveFromlocal_verified_3feeds.pid
if [ -f $PIDFILE ]
then
  PID=$(cat $PIDFILE)
...
java -Xmx30g -Xms30g -Dlog4j.configurationFile=/mnt/beegfs_bsl/tools/fileOps/log4j2.xml -jar /mnt/beegfs_bsl/tools/fileOps/FileOps_2.11-0.1-SNAPSHOT_hdfs.jar -in /data/data_lz/FdiBase/verified/ -out /mnt/beegfs_bsl/production/movefromlocaltodfs/report_new/verified/${currDate}/ -mv /mnt/beegfs_bsl/verified/ -od -of -tf 1  -nt 32 -dp 15 -re -rcm -rfl -ma -nosim -effl "^.*\.(_COPYING_|tmp|TMP)$" -effl "^\..*" -edfl "CCN_GPRS_AC" 2>&1 | tee "/mnt/beegfs_bsl/production/logs/${currDate}/verfidata_${currDate}_$mytime.log"
