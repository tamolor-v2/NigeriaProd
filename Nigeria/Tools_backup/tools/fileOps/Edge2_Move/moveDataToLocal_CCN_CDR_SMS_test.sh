...
mkdir -p /mnt/beegfs_bsl/production/movefromlocaltodfs/report/CCN_CDR_SMS/${currDate}
java -Xmx30g -Xms30g -Dlog4j.configurationFile=/mnt/beegfs/tools/fileOps/log4j2.xml -jar /mnt/beegfs/tools/fileOps/FileOps_2.11-0.1-SNAPSHOT_hdfs.jar -in /data/data_lz/beegfs/live/CCN_CDR_SMS -out /mnt/beegfs_bsl/production/movefromlocaltodfs/report/CCN_CDR_SMS/${currDate}/ -mv /mnt/beegfs_bsl/live/CCN_CDR_SMS -od -of -tf 1 -op lineCount -nt 32 -dp 15  -rcm -re -nosim -effl "^.*\.(_COPYING_|tmp|TMP)$" -effl "^\..*" 2>&1 | tee "/mnt/beegfs_bsl/production/logs/movefromlocaltodfs_CCN_CDR_SMS_${currDate}_$mytime.log"
