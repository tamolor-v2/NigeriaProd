...
PIDFILE=/home/daasuser/PIDFiles/MoveFromlocal_verified_CCN_GPRS_DA.pid
if [ -f $PIDFILE ]
then
  PID=$(cat $PIDFILE)
...
java -Xmx30g -Xms30g -Dlog4j.configurationFile=/mnt/beegfs_bsl/tools/fileOps/log4j2.xml -jar /home/daasuser/fileOps/FileOps_2.11-0.1-SNAPSHOT_hdfs.jar -in /data/data_lz/FdiBase/verified/CCN_GPRS_DA  -out /mnt/beegfs_bsl/production/movefromlocaltodfs/report_new/verified/${currDate}/ -mv /nas/share05/verified/CCN_GPRS_DA -od -of -tf 1  -nt 32 -dp 15 -re -rcm -rfl -ma -nosim -effl "^.*\.(_COPYING_|tmp|TMP)$" -effl "^\..*" 2>&1 | tee "/mnt/beegfs_bsl/production/logs/${currDate}/verfidata_${currDate}_$mytime.log"
