...
PIDFILE=/home/daasuser/PIDFiles/MoveFromlocal_DPI_NEW_crontab.pid
if [ -f $PIDFILE ]
then
  PID=$(cat $PIDFILE)
...
mkdir -p /nas/share05/production/movefromlocaltodfs/report_new/DPI_NEW/${currDate}

java -Xmx15g -Xms15g -Dlog4j.configurationFile=/nas/share05/tools/fileOps/log4j2.xml -jar /nas/share05/tools/fileOps/FileOps_2.11-0.1-SNAPSHOT_hdfs.jar -in /data/data_lz/beegfs/live/DPI_NEW -out /nas/share05/production/movefromlocaltodfs/report_new/DPI_NEW/${currDate}/ -mv /mnt/beegfs_bsl/live/DPI_NEW/ -od -of -tf 1 -op lineCount -nt 30 -dp 15   -re -rcm -rfl -nosim -effl "^.*\.(_COPYING_|tmp|TMP)$" -effl "^\..*" 2>&1 | tee "/mnt/beegfs_bsl/production/logs/movefromlocaltodfs_DPI_NEW_${currDate}_$mytime.log"
