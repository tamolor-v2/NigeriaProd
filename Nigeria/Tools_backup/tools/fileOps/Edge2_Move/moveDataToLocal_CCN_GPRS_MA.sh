...
PIDFILE=/home/daasuser/PIDFiles/MoveFromlocal_CCN_GPRS_MA_crontab_lz.pid
if [ -f $PIDFILE ]
then
  PID=$(cat $PIDFILE)
...
mkdir -p /mnt/beegfs_bsl/production/movefromlocaltodfs/report_new/CCN_GPRS_MA/${currDate}
java -Xmx30g -Xms30g -Dlog4j.configurationFile=/mnt/beegfs_bsl/tools/fileOps/log4j2.xml -jar /mnt/beegfs_bsl/tools/fileOps/FileOps_2.11-0.1-SNAPSHOT_hdfs.jar -in /data/data_lz/beegfs/live/CCN_GPRS_MA -out /mnt/beegfs_bsl/production/movefromlocaltodfs/report_new/CCN_GPRS_MA/${currDate}/ -mv /mnt/beegfs_bsl/live/CCN_GPRS_MA -od -of -tf 1 -op lineCount -nt 32 -dp 15   -re -rcm -rfl -nosim -effl "^.*\.(_COPYING_|tmp|TMP)$" -effl "^\..*"  2>&1 | tee "/mnt/beegfs_bsl/production/logs/movefromlocaltodfs_CCN_GPRS_MA_${currDate}_$mytime.log"
