...
PIDFILE=/home/daasuser/PIDFiles/MoveFromlocal_CS6_Unified_crontab_lz.pid
if [ -f $PIDFILE ]
then
  PID=$(cat $PIDFILE)
...
mkdir -p /mnt/beegfs_bsl/production/movefromlocaltodfs/report_new/CS6_Unified/${currDate}
java -Xmx30g -Xms30g -Dlog4j.configurationFile=/nas/share05/tools/fileOps/log4j2.xml -jar /nas/share05/tools/fileOps/FileOps_2.11-0.1-SNAPSHOT_hdfs.jar -in /data/data_lz/beegfs/live/CS6_Unified -out /nas/share05/production/movefromlocaltodfs/report_new/CS6_Unified/${currDate}/ -mv /mnt/beegfs_bsl/live/CS6_Unified -od -of -tf 1 -op lineCount -nt 30 -dp 15   -re -rcm -rfl -nosim -edfl "AIR_CDR|SDP_CDR" -effl "^.*\.(_COPYING_|tmp|TMP)$" -effl "^\..*" 2>&1 | tee "/mnt/beegfs_bsl/production/logs/movefromlocaltodfs_CS6_Unified_${currDate}_$mytime.log"
