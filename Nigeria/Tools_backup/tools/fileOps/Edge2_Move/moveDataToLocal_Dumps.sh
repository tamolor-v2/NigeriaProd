...
PIDFILE=/home/daasuser/PIDFiles/MoveFromlocal_Dumps_lz.pid
if [ -f $PIDFILE ]
then
  PID=$(cat $PIDFILE)
...
mkdir -p /mnt/beegfs_bsl/production/movefromlocaltodfs/report/Exclude/${currDate}
java -Xmx30g -Xms30g -Dlog4j.configurationFile=/mnt/beegfs_bsl/tools/fileOps/log4j2.xml -jar /mnt/beegfs_bsl/tools/fileOps/FileOps_2.11-0.1-SNAPSHOT_hdfs.jar -in /data/data_lz/beegfs/live/FLYTXT_CAMPAIGN_EVENTS_DATA -out /mnt/beegfs_bsl/production/movefromlocaltodfs/report/Dump/${currDate}/ -mv /mnt/beegfs_bsl/live/FLYTXT_CAMPAIGN_EVENTS_DATA -od -of -tf 1 -op list -nt 4 -dp 15   -re -rcm -rfl -nosim -effl "^.*\.(_COPYING_|tmp|TMP)$" -effl "^\..*"  2>&1 | tee "/mnt/beegfs_bsl/production/logs/movefromlocaltodfs_${currDate}_$mytime.log"
