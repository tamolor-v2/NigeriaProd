...
Feed="CDR_SMS"
BaseFeed="vp_cs5_ccn_sms_ma_dasplit"
echo "started for ${Feed} for date: $DATE"
  ...
  q2=$(/opt/presto/bin/presto --server master01003:8999 --catalog hive5 --schema flare_8 --execute "select count(*) from ${BaseFeed} where tbl_dt=${DATE} and network_type_enrich_2<>2 and call_type_desc='outgoing'")
  q11=$(echo $q1 |  sed "s/\"//g")
  q22=$(echo $q2 |  sed "s/\"//g")
  q=$((q11-q22))
  res=$(echo $q)
