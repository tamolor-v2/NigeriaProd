...
Feed="CDR_VOICE"
BaseFeed="cs5_ccn_voice_ma"
BaseFeed2="wbs_pm_rated_cdrs"
echo "started for ${Feed} for date: $DATE"
 ...
 bash  /nas/share05/tools/Engine_Room/Insertion/Insertion.scala --configFile /nas/share05/tools/Engine_Room/Insertion/Insertion.conf --feed ${Feed} --dates $DATE 2>&1 | tee /nas/share05/tools/Engine_Room/Insertion/logs/Insert_${Feed}_$(date +%Y%m%d_%s).log
  ...
  q3=$(/opt/presto/bin/presto --server master01003:8999 --catalog hive5 --schema flare_8 --execute "select count(*) from ${BaseFeed2} where tbl_dt=${DATE} and event_direction in ('TI','I') and bnum_operator = 'MTNN'")
  q11=$(echo $q1 |  sed "s/\"//g")
  q22=$(echo $q2 |  sed "s/\"//g")
  q33=$(echo $q3 |  sed "s/\"//g")
  q=$((q11-((q22+q33))))
