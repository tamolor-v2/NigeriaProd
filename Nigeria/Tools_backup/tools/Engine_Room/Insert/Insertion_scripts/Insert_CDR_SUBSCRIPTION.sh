...
echo "started for ${Feed} for date: $DATE"
  ...
  q2=$(/opt/presto/bin/presto --server master01003:8999 --catalog hive5 --schema flare_8 --execute "select count(*) from ${BaseFeed} where tbl_dt=${DATE} and regexp_like(upper(chargingcontextid),'SCAP') and length(trim(vascode))<>0 and originatingservices in ('HWSDP','CIS')")
  q11=$(echo $q1 |  sed "s/\"//g")
  q22=$(echo $q2 |  sed "s/\"//g")
  q=$((q11-q22))
  res=$(echo $q)
