...
sum(v.output_rec_cnt) Hive_Records, --
...
sum(output_rec_cnt + dup_rec_cnt + rejected_rec_cnt)- sum(fileOps_recs_cnt) Diff , -- 
cast(try((sum(output_rec_cnt + dup_rec_cnt + rejected_rec_cnt)- sum(fileOps_recs_cnt))/sum(fileOps_recs_cnt))*100 as decimal(9,4)) Diff_Ratio , --
case when sum(fileOps_recs_cnt) <> sum(output_rec_cnt + dup_rec_cnt + rejected_rec_cnt) then 'No' else 'Yes' end Status,
0 Partition_Line,
0 Unique_MSISDN,
0 Stats_Files,
...
left outer join (select substring(fsys_msgtype,18) as feed_name, main_reason, count(*) cnt ,row_number() over (partition by substring(fsys_msgtype,18) order by count(*) desc) row_num from feeds.rejecteddata where file_date=targetdate group by substring(fsys_msgtype,18), main_reason) r on z.feed_name=r.feed_name and r.row_num = 1
