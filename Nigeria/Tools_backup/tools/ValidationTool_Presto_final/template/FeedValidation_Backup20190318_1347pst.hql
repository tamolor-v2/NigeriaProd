...
select 
  coalesce(output_dup_tbl.fl_name, rejected_tbl.fl_name) as fl_name
...
from
(
select fl_name, count(*) as rec_cnt, sum(all_recs) as all_recs
from
(
select 
  reverse(element_at(split(reverse(file_name), '/'), 1)) as fl_name
, file_offset, count(*) as all_recs
...
where tbl_dt between previousDay and nextDay and feedtype = 'FeedNameValues' and coalesce(try_cast(FeedFileterValues as int), 0) = targetdate
group by reverse(element_at(split(reverse(file_name), '/'), 1)), file_offset
...
where file_date = targetdate and msgtype = 'com.mtn.messages.FeedNameValues' and coalesce(try_cast(FeedFileterValues as int), 0) = targetdate 
...
where lines > 0 and tbl_dt between previousDay and nextDay and path like '%FeedNameCondition%') FilesOps where file_dt like 'targetdate'
) as fileOps_tbl
