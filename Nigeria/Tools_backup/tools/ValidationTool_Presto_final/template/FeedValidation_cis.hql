     ...
     (SELECT reverse(element_at(split(reverse(fsys_filename), '/'), 1)) AS fl_name,
      ...
      WHERE msgtype = 'com.mtn.messages.RejectedNameValues'
        ...
        AND (coalesce(try_cast(FeedFileterRejValues AS int), 0) = targetdate
             OR COALESCE(TRY_CAST(concat('20', substring(regexp_extract(fsys_filename, '^.*_([0-9]{6}).*$', 1), 5, 2), substring(regexp_extract(fsys_filename, '^.*_([0-9]{6}).*$', 1), 3, 2), substring(regexp_extract(fsys_filename, '^.*_([0-9]{6}).*$', 1), 1, 2)) AS INT), 0) = targetdate)
