...
where tbl_dt DistributedPartitions and coalesce(try_cast( FeedFileterValues as int), 0) = targetdate and (case when file_mod_date is null or file_mod_date = '0' then 46861 when length(file_mod_date) <= 12 then cast(to_unixtime(date_parse(file_mod_date,'%Y-%m-%d')) as bigint)*1000  else cast(file_mod_date as bigint) end) <= (DateTimeFilter - 1000 * 60 * 30)
...
AND (case when file_mod_date is null or file_mod_date = '0' then 46861 when length(file_mod_date) <= 12 then cast(to_unixtime(date_parse(file_mod_date,'%Y-%m-%d')) as bigint)*1000  else cast(file_mod_date as bigint) end) <= (DateTimeFilter - 1000 * 60 * 30)
