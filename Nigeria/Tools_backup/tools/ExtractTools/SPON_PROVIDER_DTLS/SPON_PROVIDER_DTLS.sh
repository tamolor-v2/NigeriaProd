...
hostName=$(hostname)
bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 30 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/SPON_PROVIDER_DTLS/log/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
...
bash /nas/share05/tools/ExtractTools/SPON_PROVIDER_DTLS/driver.sh /mnt/beegfs_bsl/tools/ExtractTools/SPON_PROVIDER_DTLS /mnt/beegfs_bsl/FlareData/CDR/SPON_PROVIDER_DTLS/ $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
rmdir /mnt/beegfs_bsl/FlareData/CDR/SPON_PROVIDER_DTLS/incoming/$yest
