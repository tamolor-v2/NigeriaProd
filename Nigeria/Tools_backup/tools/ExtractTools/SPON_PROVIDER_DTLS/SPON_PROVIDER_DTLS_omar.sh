...
hostName=$(hostname)
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/SPON_PROVIDER_DTLS/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
...
bash /mnt/beegfs_bsl/tools/ExtractTools/SPON_PROVIDER_DTLS/driver_omar.sh /mnt/beegfs_bsl/tools/ExtractTools/SPON_PROVIDER_DTLS /mnt/beegfs_bsl/tools/ExtractTools/SPON_PROVIDER_DTLS/tmp/Omar $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
#mv /mnt/beegfs_bsl/FlareData/CDR/SPON_PROVIDER_DTLS/tmp/$yest /mnt/beegfs_bsl/FlareData/CDR/SPON_PROVIDER_DTLS/incoming/
retVal=$?
if [ $retVal -eq 0 ];
then
#Send Job Finished Email
extractedRecordsNo=$(cat /mnt/beegfs_bsl/tools/ExtractTools/SPON_PROVIDER_DTLS/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
...
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/SPON_PROVIDER_DTLS/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
...
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/SPON_PROVIDER_DTLS/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
else
