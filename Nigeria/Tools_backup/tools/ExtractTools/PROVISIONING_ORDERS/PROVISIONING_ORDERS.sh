...
hostName=$(hostname)
bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/PROVISIONING_ORDERS/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
yest=$(date -d '-1 day' '+%Y%m%d')
removeDir=$(date -d '-60 day' '+%Y%m%d')
emailReceiver=$(cat /mnt/beegfs/tools/Crontab/Scripts/email.dat)
rm -r /mnt/beegfs_bsl/tools/ExtractTools/PROVISIONING_ORDERS/logs/${removeDir}
...
#ssh edge01002 " echo -e 'ronJob \"PROVISIONING_ORDERS.sh\" Started at $(date +"%T") on edge01001, for day: $yest \n' | mailx -r 'DAAS_Note_NG@edge01001.mtn.com' -s 'DAAS_Note_MTN_NG_<extract PM_RATED Started for $yest at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver'"

bash /mnt/beegfs_bsl/tools/ExtractTools/PROVISIONING_ORDERS/driver.sh /mnt/beegfs_bsl/tools/ExtractTools/PROVISIONING_ORDERS /mnt/beegfs_bsl/tools/ExtractTools/PROVISIONING_ORDERS/tmp/Aya $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
mv /mnt/beegfs_bsl/tools/ExtractTools/PROVISIONING_ORDERS/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/PROVISIONING_ORDERS/incoming
retVal=$?
if [ $retVal -eq 0 ];
then
#Send Job Finished Email
extractedRecordsNo=$(cat /mnt/beegfs_bsl/tools/ExtractTools/PROVISIONING_ORDERS/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
...
bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/PROVISIONING_ORDERS/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
...
bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/PROVISIONING_ORDERS/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
else
