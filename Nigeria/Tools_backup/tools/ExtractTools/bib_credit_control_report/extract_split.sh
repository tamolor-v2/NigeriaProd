...
hostName=$(hostname)
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 3 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/CALL_REASON/logs/$yest" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
...
echo "msisdn = $1"
bash /mnt/beegfs_bsl/tools/ExtractTools/bib_credit_control_report/driver_split.sh /mnt/beegfs_bsl/tools/ExtractTools/bib_credit_control_report /mnt/beegfs_bsl/FlareData/CDR/bib_credit_control_report/tmp/ $yest "$1"
retVal=$?
if [ $retVal -eq 0 ];
then
#mv /mnt/beegfs_bsl/FlareData/CDR/CALL_REASON/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/CALL_REASON/incoming/
...
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 3 --status 1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/CALL_REASON/logs/$yest" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
...
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 3 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/CALL_REASON/logs/$yest" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
