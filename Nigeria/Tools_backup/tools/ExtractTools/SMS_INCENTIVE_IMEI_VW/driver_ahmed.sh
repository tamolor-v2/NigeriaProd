...
for hour in "${HOURS[@]}"; do
        ...
        for anum in "${A_NUM_FILTER[@]}"; do
                ...
                file_name=".${DATE}_SMS_INCENTIVE_IMEI_VW_${hour}_00_0${anum}_ahmed.gz"
                log_file=${DATE}_SMS_INCENTIVE_IMEI_VW_${hour}_00_0${anum}_Ahmed.log

                command="bash /home/daasuser/JDBC_Tool/JDBCExtract.scala --configFile ${WORK_DIR}/config.properties --sqlFile ${WORK_DIR}/query_template_ahmed.sql --outputFile ${output_dir}/${file_name} --delimiter ${DELIMITER} --kvTemplate ${kv}"

                echo "starting command: ${command}"
                nohup  ${command} >  ${log_dir}/${log_file}  2>&1 &

#                kv="\${date}=${DATE};\${hour}=${hour};\${minS}=30;\${minE}=59;\${anum}=${anum}"
#                file_name=".${DATE}_SMS_INCENTIVE_IMEI_VW_${hour}_30_0${anum}.gz"
#                log_file=${DATE}_SMS_INCENTIVE_IMEI_VW_${hour}_30_0${anum}.log
