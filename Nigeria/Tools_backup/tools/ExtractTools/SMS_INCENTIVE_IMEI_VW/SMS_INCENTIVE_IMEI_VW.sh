...
hostName=$(hostname)
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/SMS_INCENTIVE_IMEI_VW/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
#yest=$(date -d '-1 day' '+%Y%m%d')
yest=$(date -d '-1 day' '+%Y%m%d')
removeDir=$(date -d '-60 day' '+%Y%m%d')
emailReceiver=$(cat /nas/share05/tools/Crontab/Scripts/email.dat)
rm -r /mnt/beegfs_bsl/tools/ExtractTools/SMS_INCENTIVE_IMEI_VW/logs/${removeDir}

currDate=$(date +"%Y%m%d")
generalTime=$(date +"%Y-%m-%d %H:%M:%S")
generalLogs=/nas/share05/tools/Crontab/logs/general_logs
echo "Job: exctract_SMS_INCENTIVE_IMEI_VW. Status: Started. Time: $(date +"%Y-%m-%d %H:%M:%S")" >> ${generalLogs}/${yest}/genral_logs_${yest}.log

#Send Start Job Email
#ssh edge01002 " echo -e 'ronJob \"MASTER_DATA_VIEW.sh\" Started at $(date +"%T") on edge01001, for day: $yest \n' | mailx -r 'DAAS_Note_NG@edge01001.mtn.com' -s 'DAAS_Note_MTN_NG_<extract PM_RATED Started for $yest at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver'"

bash /nas/share05/tools/ExtractTools/SMS_INCENTIVE_IMEI_VW/driver.sh /nas/share05/tools/ExtractTools/SMS_INCENTIVE_IMEI_VW /mnt/beegfs_bsl/tools/ExtractTools/SMS_INCENTIVE_IMEI_VW/tmp $yest "00" "0" $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
mv /mnt/beegfs_bsl/tools/ExtractTools/SMS_INCENTIVE_IMEI_VW/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/SMS_INCENTIVE_IMEI_VW_WEEKLY/incoming
#Send Job Finished Email
extractedRecordsNo=$(cat /mnt/beegfs_bsl/tools/ExtractTools/SMS_INCENTIVE_IMEI_VW/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
...
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/SMS_INCENTIVE_IMEI_VW/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
...
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/MASTER_DATA_VIEW/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
