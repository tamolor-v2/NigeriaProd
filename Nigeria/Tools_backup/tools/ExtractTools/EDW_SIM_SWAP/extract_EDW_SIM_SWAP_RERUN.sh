...
hostName=$(hostname)
#bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 102 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/EDW_SIM_SWAP/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
...
bash /mnt/beegfs_bsl/tools/ExtractTools/EDW_SIM_SWAP/driver_EDW_SIM_SWAP.sh /mnt/beegfs_bsl/tools/ExtractTools/EDW_SIM_SWAP/ /mnt/beegfs_bsl/FlareData/CDR/EDW_SIM_SWAP/tmp $yest
retVal=$?
if [ $retVal -eq 0 ];
then
