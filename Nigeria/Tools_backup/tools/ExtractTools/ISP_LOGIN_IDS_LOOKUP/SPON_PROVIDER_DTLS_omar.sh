...
hostName=$(hostname)
bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs/tools/ExtractTools/SPON_PROVIDER_DTLS/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
yest=$(date -d '-1 day' '+%Y%m%d')
removeDir=$(date -d '-60 day' '+%Y%m%d')
emailReceiver=$(cat /mnt/beegfs/tools/Crontab/Scripts/email.dat)
rm -r /mnt/beegfs/tools/ExtractTools/SPON_PROVIDER_DTLS/logs/${removeDir}
...
bash /mnt/beegfs/tools/ExtractTools/SPON_PROVIDER_DTLS/driver_omar.sh /mnt/beegfs/tools/ExtractTools/SPON_PROVIDER_DTLS /mnt/beegfs/tools/ExtractTools/SPON_PROVIDER_DTLS/tmp/Omar $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
#mv /mnt/beegfs/FlareData/CDR/SPON_PROVIDER_DTLS/tmp/$yest /mnt/beegfs/FlareData/CDR/SPON_PROVIDER_DTLS/incoming/
retVal=$?
if [ $retVal -eq 0 ];
then
#Send Job Finished Email
extractedRecordsNo=$(cat /mnt/beegfs/tools/ExtractTools/SPON_PROVIDER_DTLS/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
...
bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs/tools/ExtractTools/SPON_PROVIDER_DTLS/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
...
bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs/tools/ExtractTools/SPON_PROVIDER_DTLS/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
else
