...
hostName=$(hostname)
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 12 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/CB_NEWREG_BIOUPDT_POOL_DAILY/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
...
ssh edge01002 " echo -e 'CronJob \"extract_CB_NEWREG_BIOUPDT_POOL_DAILY.sh\" Started at $(date +"%T") on edge01001, for day: $yest \n' | mailx -r 'DAAS_Note_NG@edge01001.mtn.com' -s 'DAAS_Note_MTN_NG_<extract CB_NEWREG_BIOUPDT_POOL_DAILY Started for $yest at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver' "

bash /mnt/beegfs_bsl/tools/ExtractTools/CB_NEWREG_BIOUPDT_POOL_DAILY/driver_CB_NEWREG_BIOUPDT_POOL_DAILY.sh /mnt/beegfs_bsl/tools/ExtractTools/CB_NEWREG_BIOUPDT_POOL_DAILY /mnt/beegfs_bsl/FlareData/CDR/NEWREG_BIOUPDT_POOL_DAILY/tmp/ $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
...
ssh edge01002 " echo -e 'CronJob \"extract_CB_NEWREG_BIOUPDT_POOL_DAILY.sh\" Finished at $(date +"%T"), with $extractedRecordsNo extracted records, for day: $yest \n' | mailx -r 'DAAS_Note_NG@edge01001.mtn.com' -s 'DAAS_Note_MTN_NG_<extract CB_NEWREG_BIOUPDT_POOL_DAILY Finished for $yest at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver' "
echo "Job: extract_CB_NEWREG_BIOUPDT_POOL_DAILY. Status: Finished. Time: ${generalTime}" >> ${generalLogs}/${currDate}/genral_logs_${currDate}.log
 bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 12 --status 1 --hostname "$hostName" --step 1 --log_directory "//mnt/beegfs_bsl/tools/ExtractTools/CB_NEWREG_BIOUPDT_POOL_DAILY/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
ssh edge01002 " echo -e 'CronJob \"extract_CB_NEWREG_BIOUPDT_POOL_DAILY.sh\" Failed at $(date +"%T"), for day: $yest \n' | mailx -r 'DAAS_Alert_NG@edge01001.mtn.com' -s 'DAAS_Alert_MTN_NG_<extract CB_NEWREG_BIOUPDT_POOL_DAILY Failed for $yest at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver' "
...
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 12 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/CB_NEWREG_BIOUPDT_POOL_DAILY/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
