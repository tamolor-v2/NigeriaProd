...
hostName=$(hostname)
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 12 --status 0 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/NEWREG_BIOUPDT_POOL_TEST_CLUSTER/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
...
bash /nas/share05/tools/ExtractTools/NEWREG_BIOUPDT_POOL_TEST_CLUSTER/driver_CB_NEWREG_BIOUPDT_POOL_DAILY.sh /nas/share05/tools/ExtractTools/NEWREG_BIOUPDT_POOL_TEST_CLUSTER /mnt/beegfs_bsl/FlareDataTest/NEWREG_BIOUPDT_POOL_DAILY/tmp/ $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
rmdir /mnt/beegfs_bsl/FlareDataTest/NEWREG_BIOUPDT_POOL_DAILY/incoming/$yest
 mv /mnt/beegfs_bsl/FlareDataTest/NEWREG_BIOUPDT_POOL_DAILY/tmp/$yest /mnt/beegfs_bsl/FlareDataTest/NEWREG_BIOUPDT_POOL_DAILY/incoming
#Send Job Finished Email
 extractedRecordsNo=$(cat /mnt/beegfs_bsl/tools/NEWREG_BIOUPDT_POOL_TEST_CLUSTER/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
...
echo "Job: extract_CB_NEWREG_BIOUPDT_POOL_DAILY. Status: Finished. Time: ${generalTime}" >> ${generalLogs}/${currDate}/genral_logs_${currDate}.log
 bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 12 --status 1 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/NEWREG_BIOUPDT_POOL_TEST_CLUSTER/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
...
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 12 --status -1 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/NEWREG_BIOUPDT_POOL_TEST_CLUSTER/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
