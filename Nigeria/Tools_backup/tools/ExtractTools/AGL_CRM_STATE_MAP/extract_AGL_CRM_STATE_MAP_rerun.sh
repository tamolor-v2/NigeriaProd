...
ssh edge01002 "  echo -e 'CronJob \"extract_AGL_CRM_STATE_MAP.sh\" Started at $(date +"%T") on edge01001, for day: $yest \n' | mailx -r 'DAAS_Note_NG@edge01002.mtn.com' -s 'DAAS_Note_MTN_NG_<extract AGL_CRM_STATE_MAP Started for $yest at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver' "

bash /mnt/beegfs_bsl/tools/ExtractTools/AGL_CRM_STATE_MAP/driver_AGL_CRM_STATE_MAP.sh /mnt/beegfs_bsl/tools/ExtractTools/AGL_CRM_STATE_MAP /mnt/beegfs_bsl/FlareData/CDR/AGL_CRM_STATE_MAP/tmp/ $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
...
ssh edge01002 " echo -e 'CronJob \"extract_AGL_CRM_STATE_MAP.sh\" Failed at $(date +"%T"), for day: $yest \n' | mailx -r 'DAAS_Alert_NG@edge01002.mtn.com' -s 'DAAS_Alert_MTN_NG_<extract AGL_CRM_STATE_MAP Failed for $yest at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver' "
