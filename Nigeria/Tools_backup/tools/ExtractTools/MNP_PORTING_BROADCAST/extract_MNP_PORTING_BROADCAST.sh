...
week7=$(date -d '-7 day' '+%Y%m%d')
week6=$(date -d '-6 day' '+%Y%m%d')
week5=$(date -d '-5 day' '+%Y%m%d')
week4=$(date -d '-4 day' '+%Y%m%d')
week3=$(date -d '-3 day' '+%Y%m%d')
week2=$(date -d '-2 day' '+%Y%m%d')
...
ssh edge01002 " echo -e 'CronJob \"extract_MNP_PORTING_BROADCAST.sh\" Started at $(date +"%T") on edge01001, from day: $week7 until $yest\n' | mailx -r 'DAAS_note_ng@mtn.com' -s 'DAAS_Note_MTN_NG_<extract MNP_PORTING_BROADCAST Started for $yest at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver' "
########
#######
bash /nas/share05/tools/ExtractTools/MNP_PORTING_BROADCAST/driver_MNP_PORTING_BROADCAST.sh /mnt/beegfs_bsl/tools/ExtractTools/MNP_PORTING_BROADCAST /mnt/beegfs_bsl/FlareData/CDR/MNP_PORTING_BROADCAST/tmp/ $week7 $yest
retVal=$?
if [ $retVal -eq 0 ];
then
...
#  hdfs dfs -rm  /FlareData/output_8/MNP_PORTING_BROADCAST/tbl_dt=$week7/*
#  hdfs dfs -rm /FlareData/output_8/MNP_PORTING_BROADCAST/tbl_dt=$week6/*
#  hdfs dfs -rm /FlareData/output_8/MNP_PORTING_BROADCAST/tbl_dt=$week5/*
#  hdfs dfs -rm /FlareData/output_8/MNP_PORTING_BROADCAST/tbl_dt=$week4/*
#  hdfs dfs -rm /FlareData/output_8/MNP_PORTING_BROADCAST/tbl_dt=$week3/*
#  hdfs dfs -rm /FlareData/output_8/MNP_PORTING_BROADCAST/tbl_dt=$week2/*
...
ssh edge01002 " echo -e 'CronJob \"exctract_MNP_PORTING_BROADCAST.sh\" Finished at $(date +"%T"), with $extractedRecordsNo extracted records, from day: $week7 until $yest \n' | mailx -r 'DAAS_note_ng@mtn.com' -s 'DAAS_Note_MTN_NG_<extract MNP_PORTING_BROADCAST Finished from $week7 to  $yest with $extractedRecordsNo extracted records at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver'"
...
ssh edge01002 " echo -e 'CronJob \"exctract_MNP_PORTING_BROADCAST.sh\" Failed at $(date +"%T"), from day: $week7 until $yest \n' | mailx -r 'DAAS_note_ng@mtn.com' -s 'DAAS_Alert_MTN_NG_<extract MNP_PORTING_BROADCAST Failed from $week7 to  $yest at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver' "
