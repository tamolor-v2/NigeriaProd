...
ssh edge01002 " echo -e 'CronJob \"extract_CALL_REASON.sh\" Started at $(date +"%T") on edge01001, for day: $yest \n' | mailx -r 'DAAS_note_ng@mtn.com' -s 'DAAS_Note_MTN_NG_<extract CALL_REASON Started for $yest at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver' "

bash /nas/share05/tools/ExtractTools/CALL_REASON/driver_CALL_REASON_Today.sh /nas/share05/tools/ExtractTools/CALL_REASON /mnt/beegfs_bsl/FlareData/CDR/CALL_REASON/tmp/ $yest
...
hdfs dfs -rm /FlareData/output_8/CALL_REASON/tbl_dt=${yest}/*
...
extractedRecordsNo=$(cat /nas/share05/tools/ExtractTools/CALL_REASON/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
ssh edge01002 " echo -e 'CronJob \"exctract_CALL_REASON.sh\" Finished at $(date +"%T"), with $extractedRecordsNo extracted records, for day: $yest \n' | mailx -r 'DAAS_note_ng@mtn.com' -s 'DAAS_Note_MTN_NG_<extract CALL_REASON Finished with $extractedRecordsNo extracted records for $yest at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver'"
