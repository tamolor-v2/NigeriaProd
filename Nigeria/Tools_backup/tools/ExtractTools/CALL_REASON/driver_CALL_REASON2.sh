...
cd ${WORK_DIR}
        for msisdn in "${MSISDN_FILTER[@]}"; do
                ...
                command="bash /home/daasuser/JDBC_Tool/JDBCExtract.scala --configFile ${WORK_DIR}/CALL_REASON_config.properties --sqlFile ${WORK_DIR}/CALL_REASON_query_template_bk2.sql --outputFile ${output_dir}/${file_name} --delimiter ${DELIMITER} --kvTemplate ${kv}"
