...
#month_to_run=201903
nowmonth=$(date +%Y-%m)
month_to_run=$(date -d "$nowmonth-15 last month" '+%Y%m')
tbl_dt=20160101
#tbl_dt=$(date -d "-1 month -$(($(date +%d)-1)) days" +%Y%m%d)
kinit -k -t /etc/security/keytabs/daasuser.keytab daasuser@MTN.COM
#cd  /nas/share05/tools/ExtractTools/MTNBIB_PALLET_ALLSKYYYYMM_LIVE/MTNBIB_PALLET_ALLSKYYYYMM_LIVE/spool
export ORACLE_BASE=/usr/lib/oracle
export ORACLE_HOME=/usr/lib/oracle/product/11.1.0/client_1
#BIB_CTL/h872sgf#kk@'(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=10.1.218.168)(PORT=1521))(CONNECT_DATA=(SERVER=DEDICATED)(SID=BIODSP12)))'
#start_date=$1
...
do
   ...
   bash /nas/share05/tools/ExtractTools/MTNBIB_PALLET_ALLSKYYYYMM_LIVE/templete.sh $i $month_to_run
