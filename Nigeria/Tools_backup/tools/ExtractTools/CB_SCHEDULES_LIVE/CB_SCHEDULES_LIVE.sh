...
cd /mnt/beegfs_bsl/tools/ExtractTools/CB_SCHEDULES_LIVE/spool
...
#maxSeq=$(</mnt/beegfs_bsl/tools/ExtractTools/CB_SCHEDULES_LIVE/staging/maxSeq.txt)
...
if [ -z "$maxSeq" ]
	...
	echo $maxSeq >>/mnt/beegfs_bsl/tools/ExtractTools/CB_SCHEDULES_LIVE/staging/maxSeq.txt
	dt="$(date +"%H%M%S")"
	echo "time=$dt"
	sort -t"|" -k14 -o $filename $filename
	awk  -v date=${dt} -F"|" '{print > "/mnt/beegfs_bsl/tools/ExtractTools/CB_SCHEDULES_LIVE/staging/CB_SCHEDULES_LIVE_"substr($26,1,8)"_"date".txt"}' $filename
	files=(/mnt/beegfs_bsl/tools/ExtractTools/CB_SCHEDULES_LIVE/staging/CB_SCHEDULES_LIVE*)
	for var in "${files[@]}"
		do
			tbl_dt="${var:75:8}"
			re='^[0-9]+$'
			if ! [[ $tbl_dt =~ $re ]]
				then
					...
					mkdir -p /mnt/beegfs_bsl/tools/ExtractTools/CB_SCHEDULES_LIVE/old/${tbl_dt}
					mv "${var}.gz" /mnt/beegfs_bsl/tools/ExtractTools/CB_SCHEDULES_LIVE/old/${tbl_dt}/
			fi
	done
	mv $filename ${tbl_dt}_$dt_$filename
	gzip -f ${tbl_dt}_$dt_$filename 

mv /mnt/beegfs_bsl/tools/ExtractTools/CB_SCHEDULES_LIVE/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/CB_SCHEDULES_LIVE/incoming
fi
echo "Max_Seq=$maxSeq"
hive -e "msck repair table flare_8.CB_SCHEDULES_LIVE"

#awk -v date="$(date +"%Y%m%d%H%M%S")" -F"|" '{print > "/mnt/beegfs_bsl/tools/ExtractTools/CB_SCHEDULES_LIVE/staging/CB_SCHEDULES_LIVE_"substr($14,1,8)"_"date".txt"}' $filename
