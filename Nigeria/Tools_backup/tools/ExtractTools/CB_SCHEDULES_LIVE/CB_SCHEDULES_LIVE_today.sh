...
from tt_mso_1.cb_schedules 
where LAST_MODIFIED_DATE_D>to_date($maxSeq,'yyyymmddhh24miss');
spool off
quit
EOF
chmod 777 $filename
...
if [ -z "$maxSeq" ]
	...
	echo $maxSeq >>/mnt/beegfs/tools/ExtractTools/CB_SCHEDULES_LIVE/staging/maxSeq.txt
	 dt="$(date +"%H%M%S")"
	echo "time=$dt"
	sort -t"|" -k26 -o $filename $filename
		...
		do
			...
			if ! [[ $tbl_dt =~ $re ]]
				then
					...
					chmod 777 /mnt/beegfs/tools/ExtractTools/CB_SCHEDULES_LIVE/old/${tbl_dt}
					mv "${var}.gz" /mnt/beegfs/tools/ExtractTools/CB_SCHEDULES_LIVE/old/${tbl_dt}/
					chmod 777 /mnt/beegfs/tools/ExtractTools/CB_SCHEDULES_LIVE/old/${tbl_dt}/"${var}.gz"
			fi
	done
