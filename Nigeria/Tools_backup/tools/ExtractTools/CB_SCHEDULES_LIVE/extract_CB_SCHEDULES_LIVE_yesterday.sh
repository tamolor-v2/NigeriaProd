...
hostName=$(hostname)
bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 17 --status 0 --hostname "$hostName" --step 1 --brokers $kafkaHostList --topic $TopicName
### push to kafka here
...
bash /mnt/beegfs/tools/ExtractTools/SMS_ACTIVATION_REQUEST_LIVE/SMS_ACTIVATION_REQUEST_LIVE.sh ${yest}
retVal=$?
if [ $retVal -eq 0 ];
then
...
hadoop fs -put /mnt/beegfs/tools/ExtractTools/SMS_ACTIVATION_REQUEST_LIVE/spool/wbs_bib_report_${yest}.csv.gz /FlareData/output_8/SMS_ACTIVATION_REQUEST_LIVE/tbl_dt=${yest}/
hive -e "msck repair table flare_8.SMS_ACTIVATION_REQUEST_LIVE"
echo "running :hadoop fs -put /mnt/beegfs/tools/ExtractTools/SMS_ACTIVATION_REQUEST_LIVE/spool/wbs_bib_report_${yest} /FlareData/output_8/SMS_ACTIVATION_REQUEST_LIVE/tbl_dt=${yest}/"
...
#echo "Job: extract_SMS_ACTIVATION_REQUEST_LIVE. Status: Finished. Time: $(date +"%Y-%m-%d %H:%M:%S")" >> ${generalLogs}/${currDate}/genral_logs_${currDate}.log
 bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 17 --status 1 --hostname "$hostName" --step 1 --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
