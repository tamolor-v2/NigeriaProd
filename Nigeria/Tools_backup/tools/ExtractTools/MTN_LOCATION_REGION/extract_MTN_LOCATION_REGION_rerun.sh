...
hostName=$(hostname)
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 12 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/MTN_LOCATION_REGION/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
...
bash /mnt/beegfs_bsl/tools/ExtractTools/MTN_LOCATION_REGION/driver_MTN_LOCATION_REGION.sh /mnt/beegfs_bsl/tools/ExtractTools/MTN_LOCATION_REGION /mnt/beegfs_bsl/FlareData/CDR/NEWREG_BIOUPDT_POOL_DAILY/tmp/ $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
...
echo "Job: extract_MTN_LOCATION_REGION. Status: Finished. Time: ${generalTime}" >> ${generalLogs}/${currDate}/genral_logs_${currDate}.log
 bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 12 --status 1 --hostname "$hostName" --step 1 --log_directory "//mnt/beegfs_bsl/tools/ExtractTools/MTN_LOCATION_REGION/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
...
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 12 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/MTN_LOCATION_REGION/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
