...
hostName=$(hostname)
#bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/CB_RETAIL_OUTLETS/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
yest=$(date -d '-1 day' '+%Y%m%d')
removeDir=$(date -d '-60 day' '+%Y%m%d')
emailReceiver=$(cat /mnt/beegfs/tools/Crontab/Scripts/email.dat)
rm -r /nas/share05/tools/ExtractTools/MTN_CUST_AVAILABLE_CREDIT/logs/${removeDir}
...
bash /nas/share05/tools/ExtractTools/scripts/replaceDump.sh MTN_CUST_AVAILABLE_CREDIT ${yest}
...
bash /nas/share05/tools/ExtractTools/MTN_CUST_AVAILABLE_CREDIT/driver.sh /nas/share05/tools/ExtractTools/MTN_CUST_AVAILABLE_CREDIT /nas/share05/tools/ExtractTools/MTN_CUST_AVAILABLE_CREDIT/tmp $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
mv /nas/share05/tools/ExtractTools/MTN_CUST_AVAILABLE_CREDIT/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/MTN_CUST_AVAILABLE_CREDIT/incoming
retVal=$?
if [ $retVal -eq 0 ];
then
...
#bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/CB_RETAIL_OUTLETS/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
echo "hi"
...
#bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/CB_RETAIL_OUTLETS/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
else
echo "hi"
