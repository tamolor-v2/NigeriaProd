...
hostName=$(hostname)
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/PORT_IN_OUT/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
export JAVA_OPTS="-Xms3G -Xmx20G"
export LD_BIND_NOW=1
yest=$(date -d '-1 day' '+%Y%m%d')
cd /nas/share05/tools/ExtractTools/PORT_IN_OUT/
removeDir=$(date -d '-60 day' '+%Y%m%d')
emailReceiver=$(cat /nas/share05/tools/Crontab/Scripts/email.dat)
rm -r /nas/share05/tools/ExtractTools/PORT_IN_OUT/logs/${removeDir}
...
#else 
currDate=$(date -d '-3 day' '+%Y%m%d')
echo "Running for yesterrday"
#fi
echo "running for $currDate"
echo "$currDate $runTime"
filename=/nas/share05/tools/ExtractTools/PORT_IN_OUT/tmp_max_sq_${currDate}.dat
#currDate=$(date +"%Y%m%d")
max_seq_file_name=/nas/share05/tools/ExtractTools/PORT_IN_OUT/staging/maxSeq_${currDate}.dat
...
find /mnt/beegfs_bsl/live/DB_extract_lz/PORT_IN_OUT/incoming/ -type f -name '.*' -execdir sh -c 'mv -i "$0" "./${0#./.}"' {} \;
...
#maxSeq=$( tail -n 1 /nas/share05/tools/ExtractTools/PORT_IN_OUT/staging/maxSeq_${today}.txt)
...
maxSeq=$prevMaxSeq
echo "max_seq=$maxSeq"
bash /nas/share05/tools/ExtractTools/PORT_IN_OUT/INC_driver_localJar_20190417.sh /nas/share05/tools/ExtractTools/PORT_IN_OUT /mnt/beegfs_bsl/FlareData/CDR/PORT_IN_OUT/tmp/ $currDate  "" $maxSeq $currMaxSeq
...
rmdir /mnt/beegfs_bsl/FlareData/CDR/PORT_IN_OUT/incoming/$currDate
rmdir /mnt/beegfs_bsl/live/DB_extract_lz/PORT_IN_OUT/incoming/$currDate
mv /mnt/beegfs_bsl/FlareData/CDR/PORT_IN_OUT/tmp/$currDate /mnt/beegfs_bsl/live/DB_extract_lz/PORT_IN_OUT/incoming/
retVal=$?
echo "main-Return Value: $retVal"
if [ $retVal -eq 0 ];
then
#Send Job Finished Email
extractedRecordsNo=$(cat /nas/share05/tools/ExtractTools/PORT_IN_OUT/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
...
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/PORT_IN_OUT/logs/$yest/" --general_message "$extractedRecordsNo extracted records"  --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
...
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/PORT_IN_OUT/logs/$yest/"  --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
else
