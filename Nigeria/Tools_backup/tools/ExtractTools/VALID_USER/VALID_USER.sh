...
#bash /nas/share05/tools/ExtractTools/scripts/replaceDump.sh VALID_USER ${yest} ${yest}

bash /nas/share05/tools/ExtractTools/VALID_USER/driver.sh /nas/share05/tools/ExtractTools/VALID_USER /nas/share05/tools/ExtractTools/VALID_USER/tmp $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
yest=$(date -d '-1 day' '+%Y%m%d')
hdfs dfs -rm /user/mtn_user/kamanja_test/VALID_USER/tbl_dt=$yest/*
rmdir /mnt/beegfs_bsl/live/DB_extract_lz/VALID_USER/incoming/$yest
rmdir /mnt/beegfs_bsl/FlareData/CDR/VALID_USER/incoming/$yest 
mv /nas/share05/tools/ExtractTools/VALID_USER/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/VALID_USER/incoming
retVal=$?
if [ $retVal -eq 0 ];
then
echo "finished"
