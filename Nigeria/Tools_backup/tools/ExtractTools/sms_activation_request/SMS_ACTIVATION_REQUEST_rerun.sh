...
hostName=$(hostname)
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/SMS_ACTIVATION_REQUEST/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
yest=$1
...
rm -r /nas/share05/tools/ExtractTools/SMS_ACTIVATION_REQUEST/logs/${yest}

currDate=$(date +"%Y%m%d")


# bash /nas/share05/tools/ExtractTools/scripts/replaceDump.sh SMS_ACTIVATION_REQUEST ${yest}  

#Send Start Job Email
#ssh edge01002 " echo -e 'ronJob \"SMS_ACTIVATION_REQUEST.sh\" Started at $(date +"%T") on edge01001, for day: $yest \n' | mailx -r 'DAAS_Note_NG@edge01001.mtn.com' -s 'DAAS_Note_MTN_NG_<extract PM_RATED Started for $yest at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver'"

bash /nas/share05/tools/ExtractTools/SMS_ACTIVATION_REQUEST/driver.sh /nas/share05/tools/ExtractTools/SMS_ACTIVATION_REQUEST /nas/share05/tools/ExtractTools/SMS_ACTIVATION_REQUEST/tmp/ $yest
retVal=$?
if [ $retVal -eq 0 ];
then
#echo "exit"
#exit
rmdir /mnt/beegfs_bsl/live/DB_extract_lz/SMS_ACTIVATION_REQUEST/incoming/$yest
rmdir /mnt/beegfs_bsl/FlareData/CDR/SMS_ACTIVATION_REQUEST/incoming/$yest

mv /nas/share05/tools/ExtractTools/SMS_ACTIVATION_REQUEST/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/SMS_ACTIVATION_REQUEST_KPI/incoming/
# mv /nas/share05/tools/ExtractTools/SMS_ACTIVATION_REQUEST/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/SMS_ACTIVATION_REQUEST/incoming/
retVal=$?
if [ $retVal -eq 0 ];
then
#Send Job Finished Email
extractedRecordsNo=$(cat /nas/share05/tools/ExtractTools/SMS_ACTIVATION_REQUEST/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
...
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/SMS_ACTIVATION_REQUEST/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
echo "hi"
...
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/SMS_ACTIVATION_REQUEST/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
else
echo "hi"
