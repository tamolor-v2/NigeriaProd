...
hostName=$(hostname)
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/MASTER_DATA_VIEW/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
yest=$(date -d '-1 day' '+%Y%m%d')
removeDir=$(date -d '-60 day' '+%Y%m%d')
emailReceiver=$(cat /nas/share05/tools/Crontab/Scripts/email.dat)
rm -r /nas/share05/tools/ExtractTools/MASTER_DATA_VIEW/logs/${removeDir}
...
bash /nas/share05/tools/ExtractTools/scripts/replaceDump.sh MASTER_DATA_VIEW ${yest}  


bash /nas/share05/tools/ExtractTools/MASTER_DATA_VIEW/driver.sh /nas/share05/tools/ExtractTools/MASTER_DATA_VIEW /nas/share05/tools/ExtractTools/MASTER_DATA_VIEW/tmp $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
mv /nas/share05/tools/ExtractTools/MASTER_DATA_VIEW/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/MASTER_DATA_VIEW/incoming
#Send Job Finished Email
extractedRecordsNo=$(cat /nas/share05/tools/ExtractTools/MASTER_DATA_VIEW/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
...
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/MASTER_DATA_VIEW/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
...
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/MASTER_DATA_VIEW/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
