...
hostName=$(hostname)
#bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/CB_RETAIL_OUTLETS/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
yest=$(date -d '-1 day' '+%Y%m%d')
removeDir=$(date -d '-60 day' '+%Y%m%d')
emailReceiver=$(cat /mnt/beegfs/tools/Crontab/Scripts/email.dat)
rm -r /mnt/beegfs_bsl/tools/ExtractTools/CB_RETAIL_OUTLETS/logs/${removeDir}
...
#bash /nas/share05/tools/ExtractTools/scripts/replaceDump.sh MTN_CONNECT_USER_DETAILS.sh ${yest} ${yest}

bash /nas/share05/tools/ExtractTools/MTN_CONNECT_USER_DETAILS/driver.sh /nas/share05/tools/ExtractTools/MTN_CONNECT_USER_DETAILS /nas/share05/tools/ExtractTools/MTN_CONNECT_USER_DETAILS/tmp $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
yest=$(date -d '-1 day' '+%Y%m%d')
hdfs dfs -rm /user/mtn_user/kamanja_test/MTN_CONNECT_USER_DETAILS/tbl_dt=$yest/*
rmdir /mnt/beegfs_bsl/live/DB_extract_lz/MTN_CONNECT_USER_DETAILS/incoming/$yest
rmdir /mnt/beegfs_bsl/FlareData/CDR/MTN_CONNECT_USER_DETAILS/$yest
mv /nas/share05/tools/ExtractTools/MTN_CONNECT_USER_DETAILS/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/MTN_CONNECT_USER_DETAILS/incoming
retVal=$?
if [ $retVal -eq 0 ];
then
...
#bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/CB_RETAIL_OUTLETS/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
echo "hi"
...
#bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/CB_RETAIL_OUTLETS/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
else
echo "hi"
