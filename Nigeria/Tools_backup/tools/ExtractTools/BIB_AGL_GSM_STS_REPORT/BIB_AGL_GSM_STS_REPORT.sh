...
hostName=$(hostname)
bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/BIB_AGL_GSM_STS_REPORT/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
yest=$(date -d '-1 day' '+%Y%m%d')
removeDir=$(date -d '-60 day' '+%Y%m%d')
emailReceiver=$(cat /mnt/beegfs_bsl/tools/Crontab/Scripts/email.dat)
rm -r /mnt/beegfs_bsl/tools/ExtractTools/BIB_AGL_GSM_STS_REPORT/logs/${removeDir}
...
bash /nas/share05/tools/ExtractTools/BIB_AGL_GSM_STS_REPORT/driver.sh /mnt/beegfs_bsl/tools/ExtractTools/BIB_AGL_GSM_STS_REPORT /mnt/beegfs_bsl/tools/ExtractTools/BIB_AGL_GSM_STS_REPORT/tmp $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
mv /mnt/beegfs_bsl/tools/ExtractTools/BIB_AGL_GSM_STS_REPORT/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/BIB_AGL_GSM_STS_REPORT/incoming/ 
retVal=$?
if [ $retVal -eq 0 ];
then
...
bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/CB_RETAIL_OUTLETS/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
...
bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/CB_RETAIL_OUTLETS/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
else
