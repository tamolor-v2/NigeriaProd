...
rm -r /nas/share05/tools/ExtractTools/ENROLLMENT_REF/logs/${removeDir}
...
bash /nas/share05/tools/ExtractTools/scripts/replaceDump.sh ENROLLMENT_REF ${yest}  


bash /nas/share05/tools/ExtractTools/ENROLLMENT_REF/driver.sh /nas/share05/tools/ExtractTools/ENROLLMENT_REF /nas/share05/tools/ExtractTools/ENROLLMENT_REF/tmp $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
mv /nas/share05/tools/ExtractTools/ENROLLMENT_REF/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/ENROLLMENT_REF/incoming
retVal=$?
if [ $retVal -eq 0 ];
then
...
3bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/nas/share05/tools/ExtractTools/CB_RETAIL_OUTLETS/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
else
echo "hi"
