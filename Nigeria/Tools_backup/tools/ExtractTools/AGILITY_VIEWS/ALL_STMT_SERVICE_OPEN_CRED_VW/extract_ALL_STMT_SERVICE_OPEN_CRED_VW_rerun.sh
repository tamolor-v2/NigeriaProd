...
hostName=$(hostname)
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 12 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/ALL_STMT_SERVICE_OPEN_CRED_VW/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
...
bash /mnt/beegfs_bsl/tools/ExtractTools/ALL_STMT_SERVICE_OPEN_CRED_VW/driver_ALL_STMT_SERVICE_OPEN_CRED_VW.sh /mnt/beegfs_bsl/tools/ExtractTools/ALL_STMT_SERVICE_OPEN_CRED_VW /mnt/beegfs_bsl/FlareData/CDR/NEWREG_BIOUPDT_POOL_DAILY/tmp/ $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
...
echo "Job: extract_ALL_STMT_SERVICE_OPEN_CRED_VW. Status: Finished. Time: ${generalTime}" >> ${generalLogs}/${currDate}/genral_logs_${currDate}.log
 bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 12 --status 1 --hostname "$hostName" --step 1 --log_directory "//mnt/beegfs_bsl/tools/ExtractTools/ALL_STMT_SERVICE_OPEN_CRED_VW/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
...
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 12 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/ALL_STMT_SERVICE_OPEN_CRED_VW/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
