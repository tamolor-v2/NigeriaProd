...
hostName=$(hostname)
#bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/tbl_imei_registration_dtls_vw/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
#yest=$(date -d '-1 day' '+%Y%m%d')
yest=$1
removeDir=$(date -d '-60 day' '+%Y%m%d')
emailReceiver=$(cat /mnt/beegfs_bsl/tools/Crontab/Scripts/email.dat)
rm -r /nas/share05/tools/ExtractTools/tbl_imei_registration_dtls_vw/logs/${removeDir}
...
bash /nas/share05/tools/ExtractTools/tbl_imei_registration_dtls_vw/driver.sh /nas/share05/tools/ExtractTools/tbl_imei_registration_dtls_vw /nas/share05/tools/ExtractTools/tbl_imei_registration_dtls_vw/tmp $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
...
extractedRecordsNo=$(cat /mnt/beegfs_bsl/tools/ExtractTools/tbl_imei_registration_dtls_vw/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
...
#bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/tbl_imei_registration_dtls_vw/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
...
#bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/MASTER_DATA_VIEW/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
