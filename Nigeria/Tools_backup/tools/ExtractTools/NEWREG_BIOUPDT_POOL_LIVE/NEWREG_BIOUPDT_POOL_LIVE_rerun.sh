...
select 
trim(to_char(SEQ_NO_N,'9999999999'))||'|'||
MSISDN_V||'|'||
234||''||MSISDN_V||'|'||
LAST_NAME_V||'|'||
SIM_NUMBER_V||'|'||
...
EYEBALL_RECURRINGIMG_V 
	...
	where updated_dt between to_date('${yyyymmdd} 00:00:00','yyyymmdd hh24:mi:ss') and 
	to_date('${yyyymmdd} 23:59:59','yyyymmdd hh24:mi:ss') and SEQ_NO_N>$maxSeq
	order by updated_dt;
spool off
quit
EOF
