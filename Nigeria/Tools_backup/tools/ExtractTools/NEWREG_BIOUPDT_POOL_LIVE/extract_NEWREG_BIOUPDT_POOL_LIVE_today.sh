...
bash /mnt/beegfs_bsl/tools/ExtractTools/NEWREG_BIOUPDT_POOL_LIVE/NEWREG_BIOUPDT_POOL_LIVE_today.sh ${currDate}
retVal=$?
echo "ret2=$retVal"
if [ $retVal -eq 0 ];
then
...
i#hadoop fs -rm /FlareData/output_8/NEWREG_BIOUPDT_POOL_LIVE/tbl_dt=${currDate}/*
...
#echo "Job: extract_NEWREG_BIOUPDT_POOL_LIVE. Status: Finished. Time: $(date +"%Y-%m-%d %H:%M:%S")" >> ${generalLogs}/${currDate}/genral_logs_${currDate}.log
 #bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 1 --status 1 --hostname "$hostName" --step 1 --brokers $kafkaHostList --topic $TopicName
### push to kafka here
