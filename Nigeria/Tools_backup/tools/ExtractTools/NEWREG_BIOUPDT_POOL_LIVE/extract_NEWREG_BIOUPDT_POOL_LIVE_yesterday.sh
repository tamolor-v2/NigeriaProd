...
bash /mnt/beegfs_bsl/tools/ExtractTools/NEWREG_BIOUPDT_POOL_LIVE/NEWREG_BIOUPDT_POOL_LIVE.sh ${yest}
retVal=$?
if [ $retVal -eq 0 ];
then
#Send Job Finished Email
hadoop fs -mkdir -p /FlareData/output_8/NEWREG_BIOUPDT_POOL_LIVE/tbl_dt=${yest}
echo "running: hadoop fs -rm /FlareData/output_8/NEWREG_BIOUPDT_POOL_LIVE/tbl_dt=${yest}/*"
hadoop fs -rm /FlareData/output_8/NEWREG_BIOUPDT_POOL_LIVE/tbl_dt=${yest}/*
hadoop fs -put /mnt/beegfs_bsl/tools/ExtractTools/NEWREG_BIOUPDT_POOL_LIVE/spool/wbs_bib_report_${yest}.csv.gz /FlareData/output_8/NEWREG_BIOUPDT_POOL_LIVE/tbl_dt=${yest}/
hive -e "msck repair table flare_8.NEWREG_BIOUPDT_POOL_LIVE"
echo "running :hadoop fs -put /mnt/beegfs_bsl/tools/ExtractTools/NEWREG_BIOUPDT_POOL_LIVE/spool/wbs_bib_report_${yest} /FlareData/output_8/NEWREG_BIOUPDT_POOL_LIVE/tbl_dt=${yest}/"
#echo "mv /mnt/beegfs_bsl/tools/ExtractTools/NEWREG_BIOUPDT_POOL_LIVE/spool/wbs_bib_report_${yest}.csv.gz /mnt/beegfs_bsl/tools/ExtractTools/NEWREG_BIOUPDT_POOL_LIVE/old/"
#mv /mnt/beegfs_bsl/tools/ExtractTools/NEWREG_BIOUPDT_POOL_LIVE/spool/wbs_bib_report_${yest}.csv.gz /mnt/beegfs_bsl/tools/ExtractTools/NEWREG_BIOUPDT_POOL_LIVE/old/
# extractedRecordsNo=$(cat /mnt/beegfs_bsl/tools/ExtractTools/NEWREG_BIOUPDT_POOL_LIVE/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
#ssh edge01002 " echo -e 'CronJob \"extract_AGL_CRM_LGA_MAP.sh\" Finished at $(date +"%T"), with $extractedRecordsNo extracted records, for day: $yest \n' | mailx -r 'DAAS_Note_NG@edge01002.mtn.com' -s 'DAAS_Note_MTN_NG_<extract NEWREG_BIOUPDT_POOL_LIVE Finished for $yest with $extractedRecordsNo extracted records at $(date +"%Y-%m-%d %H:%M:%S")' '$emailReceiver' "
...
#ssh edge01002 " echo -e 'CronJob \"extract_NEWREG_BIOUPDT_POOL_LIVE.sh\" Failed at $(date +"%T"), for day: $yest \n' | mailx -r 'DAAS_Alert_NG@edge01002.mtn.com' -s 'DAAS_Alert_MTN_NG_<extract NEWREG_BIOUPDT_POOL_LIVE Failed for $yest at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver' "
