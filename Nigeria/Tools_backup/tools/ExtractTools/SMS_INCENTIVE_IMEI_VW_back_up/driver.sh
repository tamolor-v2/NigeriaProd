...
for hour in "${HOURS[@]}"; do
        ...
        for anum in "${A_NUM_FILTER[@]}"; do
                ...
                kv="\${date}=${DATE};\${hour}=${hour};\${minS}=00;\${minE}=29;\${anum}=${anum};\${history_date}=${history_date}"
                file_name=".${DATE}_SMS_INCENTIVE_IMEI_VW_${hour}_00_0${anum}.gz"
                log_file=${DATE}_SMS_INCENTIVE_IMEI_VW_${hour}_00_0${anum}.log
