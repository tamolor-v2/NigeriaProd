...
hostName=$(hostname)
#bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/CREDIT_INFORMATION_MON/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
yest=$(date -d '-1 day' '+%Y%m%d')
removeDir=$(date -d '-60 day' '+%Y%m%d')
emailReceiver=$(cat /mnt/beegfs/tools/Crontab/Scripts/email.dat)
rm -r /mnt/beegfs_bsl/tools/ExtractTools/CREDIT_INFORMATION_MON/logs/${removeDir}
...
#ssh edge01002 " echo -e 'ronJob \"CREDIT_INFORMATION_MON.sh\" Started at $(date +"%T") on edge01001, for day: $yest \n' | mailx -r 'DAAS_Note_NG@edge01001.mtn.com' -s 'DAAS_Note_MTN_NG_<extract PM_RATED Started for $yest at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver'"
echo "extracting"
bash /mnt/beegfs_bsl/tools/ExtractTools/CREDIT_INFORMATION_MON/driver.sh /mnt/beegfs_bsl/tools/ExtractTools/CREDIT_INFORMATION_MON /mnt/beegfs_bsl/tools/ExtractTools/CREDIT_INFORMATION_MON/tmp $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
mv /mnt/beegfs_bsl/tools/ExtractTools/CREDIT_INFORMATION_MON/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/CREDIT_INFORMATION_MON/incoming/
retVal=$?
if [ $retVal -eq 0 ];
then
#Send Job Finished Email
extractedRecordsNo=$(cat /mnt/beegfs_bsl/tools/ExtractTools/CREDIT_INFORMATION_MON/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
...
bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/CREDIT_INFORMATION_MON/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
...
bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/CREDIT_INFORMATION_MON/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
else
