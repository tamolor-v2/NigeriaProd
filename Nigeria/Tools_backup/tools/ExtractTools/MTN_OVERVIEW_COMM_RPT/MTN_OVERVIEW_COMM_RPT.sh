...
hostName=$(hostname)
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/MTN_OVERVIEW_COMM_RPT/logs/$curr_date/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
curr_date=$(date -d '-1 day' '+%Y%m%d')
removeDir=$(date -d '-60 day' '+%Y%m%d')
emailReceiver=$(cat /nas/share05/tools/Crontab/Scripts/email.dat)
rm -r /mnt/beegfs_bsl/tools/ExtractTools/MTN_OVERVIEW_COMM_RPT/logs/${curr_date}
...
#ssh edge01002 " echo -e 'ronJob \"MTN_OVERVIEW_COMM_RPT.sh\" Started at $(date +"%T") on edge01001, for day: $curr_date \n' | mailx -r 'DAAS_Note_NG@edge01001.mtn.com' -s 'DAAS_Note_MTN_NG_<extract PM_RATED Started for $curr_date at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver'"
bash /nas/share05/tools/ExtractTools/MTN_OVERVIEW_COMM_RPT/driver.sh /mnt/beegfs_bsl/tools/ExtractTools/MTN_OVERVIEW_COMM_RPT /mnt/beegfs_bsl/tools/ExtractTools/MTN_OVERVIEW_COMM_RPT/tmp $curr_date
retVal=$?
if [ $retVal -eq 0 ];
then
hadoop fs -rm -R  /FlareData/output_8/MTN_OVERVIEW_COMM_RPT/*/*
# hive -e "ALTER TABLE Flare_8.MTN_OVERVIEW_COMM_RPT DROP IF EXISTS PARTITION (TBL_DT<>0)"
mv /mnt/beegfs_bsl/tools/ExtractTools/MTN_OVERVIEW_COMM_RPT/tmp/$curr_date /mnt/beegfs_bsl/live/DB_extract_lz/MTN_OVERVIEW_COMM_RPT/incoming/
retVal=$?
if [ $retVal -eq 0 ];
then
#Send Job Finished Email
extractedRecordsNo=$(cat /mnt/beegfs_bsl/tools/ExtractTools/MTN_OVERVIEW_COMM_RPT/logs/$curr_date/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
...
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/MTN_OVERVIEW_COMM_RPT/logs/$curr_date/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
echo "hi"
...
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/MTN_OVERVIEW_COMM_RPT/logs/$curr_date/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
else
echo "hi"
