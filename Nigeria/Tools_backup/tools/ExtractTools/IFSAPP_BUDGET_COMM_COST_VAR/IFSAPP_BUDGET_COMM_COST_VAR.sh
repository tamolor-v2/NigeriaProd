...
hostName=$(hostname)
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/IFSAPP_BUDGET_COMM_COST_VAR/logs/$curr_date/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
...
rm -r /nas/share05/tools/ExtractTools/IFSAPP_BUDGET_COMM_COST_VAR/logs/${curr_date}
...
#ssh edge01002 " echo -e 'ronJob \"IFSAPP_BUDGET_COMM_COST_VAR.sh\" Started at $(date +"%T") on edge01001, for day: $curr_date \n' | mailx -r 'DAAS_Note_NG@edge01001.mtn.com' -s 'DAAS_Note_MTN_NG_<extract PM_RATED Started for $curr_date at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver'"
bash /nas/share05/tools/ExtractTools/IFSAPP_BUDGET_COMM_COST_VAR/driver.sh /nas/share05/tools/ExtractTools/IFSAPP_BUDGET_COMM_COST_VAR /nas/share05/tools/ExtractTools/IFSAPP_BUDGET_COMM_COST_VAR/tmp $curr_date
retVal=$?
if [ $retVal -eq 0 ];
then
kinit -kt /etc/security/keytabs/daasuser.keytab daasuser@MTN.COM
hadoop fs -rm /FlareData/output_8/IFSAPP_BUDGET_COMM_COST_VAR/tbl_dt=*/*
mv /nas/share05/tools/ExtractTools/IFSAPP_BUDGET_COMM_COST_VAR/tmp/$curr_date /mnt/beegfs_bsl/live/DB_extract_lz/IFSAPP_BUDGET_COMM_COST_VAR/incoming
retVal=$?
if [ $retVal -eq 0 ];
then
#Send Job Finished Email
extractedRecordsNo=$(cat /mnt/beegfs_bsl/tools/ExtractTools/IFSAPP_BUDGET_COMM_COST_VAR/logs/$curr_date/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
...
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/IFSAPP_BUDGET_COMM_COST_VAR/logs/$curr_date/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
echo "hi"
...
#bash /nas/share05/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/IFSAPP_BUDGET_COMM_COST_VAR/logs/$curr_date/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
else
echo "hi"
