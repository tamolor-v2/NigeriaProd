...
start transaction;
delete from flare_8.cb_serv_mast_view_live where tbl_dt=yyyymmdd and msisdn_part >= 0;
...
tbl_dt,
msisdn_part
from flare_8.cb_serv_mast_view_live_inc where tbl_dt=yyyymmdd and msisdn_part > -1
union all
...
tbl_dt,
msisdn_part
from flare_8.cb_serv_mast_view_live where tbl_dt=cast(date_format(date_add('day',-1,date_parse(cast(yyyymmdd as varchar),'%Y%m%d')),'%Y%m%d') as int) and msisdn_part > -1
) 
) where rnk=1;
commit;
