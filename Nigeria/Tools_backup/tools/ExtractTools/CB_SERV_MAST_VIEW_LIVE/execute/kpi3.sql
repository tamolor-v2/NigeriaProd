start transaction;
delete from kpi.kpi_val where kpi_id='KPI-000003' and tbl_dt=20190108;
insert into kpi.kpi_val
select
cast(20190108 as int) as date_key,
cast('KPI-000003' as varchar) as kpi_name,
cast('Subscriber growth %' as varchar) as kpi_desc,
cast(0 as bigint) as kpi000001,
cast(0 as int) as kpi000002,
cast(round(100*(net_adds/opening_base),4) as double) as kpi000003,
...
(select cast(count(*)*1.0 as double) net_adds
from (
select closing.msisdn_key
...
and (opening.is_in_today_sdp or opening.is_in_prevdays_sdp) and closing.msisdn_key=opening.msisdn_key))) net_additions;
commit;
