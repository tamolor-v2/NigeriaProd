start transaction;
delete from kpi.kpi_val where kpi_id='KPI-000019' and tbl_dt=20190108;
insert into kpi.kpi_val
select
cast(20190108 as int) as date_key,
cast('KPI-000019' as varchar) as kpi_name,
cast('Data Subscribers Growth' as varchar) as kpi_desc,
...
cast(round((close_data_subs - open_data_subs)/open_data_subs,4) as double) as kpi000019,
...
from
(
select cast(count(distinct msisdn_key)*1.0 as double) open_data_subs from (
select
ce.tbl_dt,
ce.msisdn_key,
ce.last_dt,
...
where et.rgs_ind='Y' and ce.event_type=et.event_type 
and ce.tbl_dt=cast(date_format(DATE_ADD('day', -1, date_trunc('month', date_parse(cast(20190108 as varchar),'%Y%m%d'))),'%Y%m%d') as int)
and ce.aggr='daily' and ce.aggr=cs.aggr and ce.tbl_dt=cs.tbl_dt
...
) opening_data_subscribers,
(
select cast(count(distinct msisdn_key)*1.0 as double) close_data_subs from (
select
ce.tbl_dt,
ce.msisdn_key,
ce.last_dt,
