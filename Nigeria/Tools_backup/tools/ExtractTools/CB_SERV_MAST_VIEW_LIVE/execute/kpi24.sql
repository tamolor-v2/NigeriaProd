start transaction;
delete from kpi.kpi_val where kpi_id='KPI-000024' and tbl_dt=20190108;
insert into kpi.kpi_val
select
cast(20190108 as int) as date_key,
cast('KPI-000024' as varchar) as kpi_name,
cast('2G % of total handsets' as varchar) as kpi_desc,
...
cast(round(100.0*(device_2g_devicecount/tot_uniq_devicecount),2) as double) as kpi000024,
...
from (
select
cast(count(distinct case when a.nw_2g_ind=1 and a.nw_3g_ind<>1 and a.lte_4g_ind<>1 then imei else '0' end)*1.0 as double)-1 device_2g_devicecount,
