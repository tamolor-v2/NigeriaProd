		...
		do
			...
			if ! [[ $tbl_dt =~ $re ]]
				then
					...
					mkdir -p /mnt/beegfs_bsl/live/DB_extract_lz/CB_SERV_MAST_VIEW_LIVE/incoming/${tbl_dt}
					mv "${var}.gz" /mnt/beegfs_bsl/live/DB_extract_lz/CB_SERV_MAST_VIEW_LIVE/incoming/${tbl_dt}/
			fi
	done
	mv $filename ${tbl_dt}_$dt_$filename
	gzip -f ${tbl_dt}_$dt_$filename 

mv /mnt/beegfs_bsl/tools/ExtractTools/CB_SERV_MAST_VIEW_LIVE/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/CB_SERV_MAST_VIEW_LIVE/incoming
fi
echo "Max_Seq=$maxSeq"
hive -e "msck repair table flare_8.CB_SERV_MAST_VIEW_LIVE"
hive -e "msck repair table feeds.CB_SERV_MAST_VIEW_LIVE"
