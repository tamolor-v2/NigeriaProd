...
'234'||MOBL_NUM_VOICE_V||'|'||
substr(MOBL_NUM_VOICE_V,-1)
from cbs_tbl_core.cb_serv_mast_view where substr(MOBL_NUM_VOICE_V,-1)='$msisdnLastDigit'
and date_d>to_date('$maxSeq','yyyymmdd hh24miss');
spool off
quit
EOF
...
hadoop fs -test -d $DIRECTORY
    if [ $? == 0 ]
		...
		echo "hadoop fs -mkdir -p /FlareData/output_8/CB_SERV_MAST_VIEW_LIVE_INC/tbl_dt=${tbl_dt}/msisdn_part=${msisdnLastDigit}"
		hadoop fs -mkdir -p /FlareData/output_8/CB_SERV_MAST_VIEW_LIVE_INC/tbl_dt=${tbl_dt}/msisdn_part=${msisdnLastDigit}
                ...
                mkdir -p /mnt/beegfs_bsl/tools/ExtractTools/CB_SERV_MAST_VIEW_LIVE/old/${tbl_dt}/${msisdnLastDigit}
		mkdir -p /mnt/beegfs_bsl/FlareData/CDR/CB_SERV_MAST_VIEW/processed/${tbl_dt}/${msisdnLastDigit}
		cp "${filename}.gz" /mnt/beegfs_bsl/FlareData/CDR/CB_SERV_MAST_VIEW/processed/${tbl_dt}/${msisdnLastDigit}
                mv "${filename}.gz" /mnt/beegfs_bsl/tools/ExtractTools/CB_SERV_MAST_VIEW_LIVE/old/${tbl_dt}/${msisdnLastDigit}
