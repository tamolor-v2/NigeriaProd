...
hostName=$(hostname)
bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/NPS_KIND_IMSI_ROUTING/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
yest=$(date -d '-1 day' '+%Y%m%d')
removeDir=$(date -d '-60 day' '+%Y%m%d')
emailReceiver=$(cat /mnt/beegfs/tools/Crontab/Scripts/email.dat)
rm -r /mnt/beegfs_bsl/tools/ExtractTools/NPS_KIND_IMSI_ROUTING/logs/${removeDir}
...
#ssh edge01002 " echo -e 'ronJob \"NPS_KIND_IMSI_ROUTING.sh\" Started at $(date +"%T") on edge01001, for day: $yest \n' | mailx -r 'DAAS_Note_NG@edge01001.mtn.com' -s 'DAAS_Note_MTN_NG_<extract PM_RATED Started for $yest at $(date +"%Y-%m-%d %H:%M:%S")>' '$emailReceiver'"

bash /mnt/beegfs_bsl/tools/ExtractTools/NPS_KIND_IMSI_ROUTING/driver.sh /mnt/beegfs_bsl/tools/ExtractTools/NPS_KIND_IMSI_ROUTING /mnt/beegfs_bsl/tools/ExtractTools/NPS_KIND_IMSI_ROUTING/tmp/Aya $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
mv /mnt/beegfs_bsl/tools/ExtractTools/NPS_KIND_IMSI_ROUTING/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/NPS_KIND_IMSI_ROUTING/incoming
retVal=$?
if [ $retVal -eq 0 ];
then
#Send Job Finished Email
extractedRecordsNo=$(cat /mnt/beegfs_bsl/tools/ExtractTools/NPS_KIND_IMSI_ROUTING/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
...
bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/NPS_KIND_IMSI_ROUTING/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
...
bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/NPS_KIND_IMSI_ROUTING/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
else
