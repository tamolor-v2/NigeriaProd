...
rm -r /mnt/beegfs_bsl/tools/ExtractTools/BILL_SUMMARY_REP_MON/logs/${removeDir}
...
bash /nas/share05/tools/ExtractTools/BILL_SUMMARY_REP_MON/driver.sh /nas/share05/tools/ExtractTools/BILL_SUMMARY_REP_MON /mnt/beegfs_bsl/tools/ExtractTools/BILL_SUMMARY_REP_MON/tmp $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
#mv /mnt/beegfs_bsl/tools/ExtractTools/BILL_SUMMARY_REP_MON/tmp/$yest /mnt/beegfs_bsl/live/DB_extract_lz/BILL_SUMMARY_REP_MON/incoming/
extractedRecordsNo=$(cat /mnt/beegfs_bsl/tools/ExtractTools/BILL_SUMMARY_REP_MON/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
echo "done done done done $extractedRecordsNo" 
retVal=$?
if [ $retVal -eq 0 ];
then
