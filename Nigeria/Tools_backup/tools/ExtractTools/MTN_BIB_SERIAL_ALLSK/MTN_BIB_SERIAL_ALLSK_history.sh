...
cd /mnt/beegfs/tools/ExtractTools/MTN_BIB_SERIAL_ALLSK/spool
...
filename="MTN_BIB_SERIAL_ALLSK_"$month"_"$yyyymmdd"_"$dt".csv"
#maxSeq=$(</mnt/beegfs/tools/ExtractTools/MTN_BIB_SERIAL_ALLSK/staging/maxSeq.txt)
...
hadoop fs -test -d $DIRECTORY
    if [ $? == 0 ]
		...
		mkdir -p /mnt/beegfs/tools/ExtractTools/MTN_BIB_SERIAL_ALLSK/old/${month}
		hadoop fs -put "${filename}.gz" /FlareData/output_8/MTN_BIB_SERIAL_ALLSK/tbl_dt=${month}
		mv "${filename}.gz" /mnt/beegfs/tools/ExtractTools/MTN_BIB_SERIAL_ALLSK/old/${month}/
