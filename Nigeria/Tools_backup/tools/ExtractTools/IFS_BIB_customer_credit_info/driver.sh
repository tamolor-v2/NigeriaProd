...
#                kv="\${date}=${DATE};\${hour}=${hour};\${minS}=00;\${minE}=29;\${anum}=${anum}"
                file_name=".${yyyymmdd}_IFS_BIB_customer_credit_info.gz"
                log_file=${yyyymmdd}_IFS_BIB_customer_credit_info.log
                kv="\${DATE}=${DATE}"
                command="bash /home/daasuser/JDBC_Tool/JDBCExtract.scala --configFile ${WORK_DIR}/config.properties --sqlFile ${WORK_DIR}/new_query_template.sql --outputFile ${output_dir}/${file_name} --delimiter ${DELIMITER} --kvTemplate ${kv}"
