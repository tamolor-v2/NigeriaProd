...
hostName=$(hostname)
#bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs/tools/ExtractTools/CB_RETAIL_OUTLETS/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
yest=$1
removeDir=$(date -d '-60 day' '+%Y%m%d')
emailReceiver=$(cat /mnt/beegfs/tools/Crontab/Scripts/email.dat)
rm -r /mnt/beegfs/tools/ExtractTools/IFS_BIB_customer_credit_info/logs/${removeDir}
...
bash /mnt/beegfs/tools/ExtractTools/IFS_BIB_customer_credit_info/driver.sh /mnt/beegfs/tools/ExtractTools/IFS_BIB_customer_credit_info /mnt/beegfs/tools/ExtractTools/IFS_BIB_customer_credit_info/tmp/ $yest 
retVal=$?
if [ $retVal -eq 0 ];
then
mv /mnt/beegfs/tools/ExtractTools/IFS_BIB_customer_credit_info/tmp/$yest /mnt/beegfs/FlareDataTest/IFS_BIB_customer_credit_info/incoming/ 
retVal=$?
if [ $retVal -eq 0 ];
then
#Send Job Finished Email
extractedRecordsNo=$(cat /mnt/beegfs/tools/ExtractTools/CB_RETAIL_OUTLETS/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
...
#bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status 1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs/tools/ExtractTools/CB_RETAIL_OUTLETS/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
echo "hi"
...
#bash /mnt/beegfs/tools/JsonBuilder/JsonBuilder.scala --id 2 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs/tools/ExtractTools/CB_RETAIL_OUTLETS/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
fi
else
echo "hi"
