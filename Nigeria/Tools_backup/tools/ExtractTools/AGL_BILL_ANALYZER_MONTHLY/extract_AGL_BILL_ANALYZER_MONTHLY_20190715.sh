...
rm -r /mnt/beegfs_bsl/tools/ExtractTools/AGL_BILL_ANALYZER_MONTHLY/logs/${removeDir}
currDate=$(date +"%Y%m%d")
...
hostName=$(hostname)
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 41 --status 0 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/AGL_BILL_ANALYZER_MONTHLY/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
...
bash /nas/share05/tools/ExtractTools/AGL_BILL_ANALYZER_MONTHLY/driver_AGL_BILL_ANALYZER_MONTHLY.sh /mnt/beegfs_bsl/tools/ExtractTools/AGL_BILL_ANALYZER_MONTHLY /mnt/beegfs_bsl/tools/ExtractTools/AGL_BILL_ANALYZER_MONTHLY/tmp/$yest 
retVal=$?
if [ $retVal -eq 0 ];
then
...
extractedRecordsNo=$(cat /mnt/beegfs_bsl/tools/ExtractTools/AGL_BILL_ANALYZER_MONTHLY/logs/$yest/*.log | grep "summary: " | more | cut -d ',' -f 2 | cut -d ' ' -f 2 | awk '{total=total+$1} END {print total}')
mv /mnt/beegfs_bsl/tools/ExtractTools/AGL_BILL_ANALYZER_MONTHLY/tmp/${yest} /mnt/beegfs_bsl/live/DB_extract_lz/AGL_BILL_ANALYZER_MONTHLY/incoming/
...
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 41 --status 1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/AGL_BILL_ANALYZER_MONTHLY/logs/$yest/" --general_message "$extractedRecordsNo extracted records" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
else
...
bash /mnt/beegfs_bsl/tools/JsonBuilder/JsonBuilder.scala --id 41 --status -1 --hostname "$hostName" --step 1 --log_directory "/mnt/beegfs_bsl/tools/ExtractTools/AGL_BILL_ANALYZER_MONTHLY/logs/$yest/" --brokers $kafkaHostList --topic $TopicName
### push to kafka here
