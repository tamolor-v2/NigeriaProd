...
echo "select /*+ parallel 12 */ max(to_timestamp('${maxSeq}','yyyymmddhh24missFF')) from wbs_client.wbs_cdr_${processDay} where ANUM is not null"
...
set wrap off
spool $filename
SELECT /*+ parallel(12)*/ TO_CHAR(MAX(PROCESS_DATE), 'yyyymmddhh24miss') FROM WBS_CLIENT.WBS_CDR_${currDate} WHERE ANUM IS NOT NULL;
spool off
quit
EOF
currMaxSeq=$( tail -n 1 ${filename})

echo ${currMaxSeq}>>/nas/share05/tools/ExtractTools/WBS_PM_RATED_CDRS/staging/${max_seq_file_name}
