...
find /nas/share05/FlareProd/ExtractLZ/WBS_PM_RATED_CDRS/incoming/ -type f -name '.*' -execdir sh -c 'mv -i "$0" "./${0#./.}"' {} \;
...
set wrap off
spool $filename
SELECT /*+ parallel(12)*/ TO_CHAR(MAX(PROCESS_DATE- 0.25/24), 'yyyymmddhh24miss') FROM WBS_CLIENT.WBS_CDR_${currDate} WHERE ANUM IS NOT NULL;
spool off
quit
EOF
...
rmdir /nas/share05/FlareProd/ExtractLZ/WBS_PM_RATED_CDRS/incoming/$currDate
mv /nas/share05/FlareProd/Working/WBS_PM_RATED_CDRS/$currDate /nas/share05/FlareProd/ExtractLZ/WBS_PM_RATED_CDRS/incoming/
retVal=$?
