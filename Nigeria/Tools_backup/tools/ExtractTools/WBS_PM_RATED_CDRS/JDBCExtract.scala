#!/bin/sh
exec scala -cp /home/daasuser/ojdbc6.jar -savecompiled "$0" "$@"
!#
  ...
  def extractToFile(query: String, outputFile: String, delimiter: String): Unit = {
    ...
    try {
      if (resultSetStatement != null) {
        var fileSeq = 1
        var curOutputFile = outputFile.replace(""".gz""", "_" + fileSeq +""".gz""")
        fileWriter = getFileWriter(curOutputFile)
        ...
        while (resultSet.next()) {
          ...
          fileWriter.write(row.map(v=>if(v!=null) v.replace("\n","").replace("\r","").replace(delimiter,"") else v).mkString(delimiter))
          counter += 1
          if (counter >= 200000) {
            val recCnt = fileWriter.getCurrentRecordCount
            fileWriter.close()
            fileWriter = null
            val endTime = System.currentTimeMillis
            val milliSec = endTime - startTime
            val sec = milliSec / 1000
            Log.logMsg(s"summary: file: ${curOutputFile},lines: ${recCnt}, stime:$startTime etime:${endTime},sec:${sec},millis:${milliSec}")
            FileHelper.renameDotFile(curOutputFile)
            startTime = endTime
            fileSeq += 1
            curOutputFile = outputFile.replace(""".gz""", "_" + fileSeq +""".gz""")
            fileWriter = getFileWriter(curOutputFile)
            counter = 0
          }
        }
