...
echo "SELECT /*+ parallel(12)*/ TO_CHAR(MAX(PROCESS_DATE), 'yyyymmddhh24miss') FROM WBS_CLIENT.WBS_CDR_${currDate} WHERE ANUM IS NOT NULL;"
...
set wrap off
spool $filename
SELECT /*+ parallel(12)*/ TO_CHAR(MAX(PROCESS_DATE+ 1/86400), 'yyyymmddhh24miss') FROM WBS_CLIENT.WBS_CDR_${currDate} WHERE ANUM IS NOT NULL;
spool off
quit
EOF
