#!/bin/sh
exec scala -cp ojdbc6.jar -savecompiled "$0" "$@"
!#
...
class DataExtractor(private val jdbcManager: JDBCManager) {
  def extractToFile(query: String, outputFile: String, delimiter: String): Unit = {
	...
	var fileSeq=1
	var prevFile=outputFile.replace(""".gz""","_"+fileSeq+""".gz""")
        fileWriter = getFileWriter(outputFile.replace(""".gz""","""_1.gz"""))
	...
	counter+=1
	if(counter>=200000){
 prevFile=outputFile.replace(""".gz""","_"+fileSeq+""".gz""")
        if (fileWriter != null)
          fileWriter.close()
        val endTime = System.currentTimeMillis
        val milliSec = endTime - startTime
        val sec = milliSec / 1000
   Log.logMsg(s"summary: file: ${prevFile},lines: ${fileWriter.getCurrentRecordCount}, stime:$startTime etime:${endTime},sec:${sec},millis:${milliSec}")
FileHelper.renameDotFile(prevFile)
		fileSeq +=1
		fileWriter = getFileWriter(outputFile.replace(""".gz""","_"+fileSeq+""".gz"""))
		counter=0
}
        }
        if (fileWriter != null){
