...
for feed in CVM20_BIO_DATA_TMP CVM20_SIM_TMP CVM20_USG_VOI_USG_CLUSTER_TMP CVM20_CUSTOMERSUBJECT_TMP CVM20_CUSTOMERINFO 
do
 echo "Started for $feed at $SYS_DATE"
 if [ $feed == "CVM20_CUSTOMERINFO" ]
 then
  ...
  if [ $feed == "CVM20_USG_VOI_USG_CLUSTER_TMP" ]
  then
    q0=$(/opt/presto/bin/presto --server master01003:8999 --catalog hive5  --schema cvm_db --execute "select count(*) from $feed where tbl_dt=${DATE}")
    q00=$(echo $q1 |  sed "s/\"//g")
    if [ $q00 == 0 ]
    then
       python3.6 /nas/share05/tools/TransactionsTool/runTransaction.py -cf /nas/share05/tools/TransactionsTool/runTransactionConfig.json -qn $feed -rd $DATE
