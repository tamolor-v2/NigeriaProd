...
for feed in CVM20_DATA_BUNDLE_REV_TMP CVM20_DIGITAL_SERVICES_REV_TMP CVM20_REV_IC_TMP CVM20_REV_VOI_SMS_DATA_TMP_STG CVM20_REV_VOI_SMS_DATA_TMP 
do
 echo "Started for $feed at $SYS_DATE"
 hdfs dfs -rm -skipTrash /FlareData/CVM_DB/${feed}/tbl_dt=${DATE}/*
 if [ $feed == "CVM20_REV_VOI_SMS_DATA_TMP" ]
 then
