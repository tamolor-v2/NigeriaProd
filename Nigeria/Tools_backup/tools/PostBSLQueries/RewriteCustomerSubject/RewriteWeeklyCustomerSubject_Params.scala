...
object RewriteCustomerSubject {
  val rewriteQuery: String =
    """
      ...
      #from flare_8.CustomerSubject where tbl_dt = ${DATA_DATE} and aggr = 'weekly' and ((vas_subscription_1 is not null and length(vas_subscription_1) > 0) or
      ...
      #on (cust.tbl_dt = ${DATA_DATE} and cust.aggr = 'weekly' and cust.msisdn_key = sdp.msisdn_key)
      #;
      #
      """.stripMargin('#')
...
//    val stDate = 20180628
//    val endDate = 20180725
//    val stDate = 20180816
//    val endDate = 20180816
println(args.toList)
parseArgs(args)
