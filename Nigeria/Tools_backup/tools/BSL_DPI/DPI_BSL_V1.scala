...
object DPI_BSL {
  ...
  val filesList=Array(
    "/home/daasuser/samer_scripts/BSL_DPI/DPI_BSL_Queries_2/dpi_bsl_subjectunpack.txt",
    "/home/daasuser/samer_scripts/BSL_DPI/DPI_BSL_Queries_2/dpi_bsl_subjectaggr.txt",
    "/home/daasuser/samer_scripts/BSL_DPI/DPI_BSL_Queries_2/dpi_bsl_subjectopco.txt",
    "/home/daasuser/samer_scripts/BSL_DPI/DPI_BSL_Queries_2/dpi_bsl_subjectpivot.txt"
  )
            ...
            for (a <- chunks) {
              ...
              if (!a.toLowerCase.startsWith("drop")) {
                if (!a.toLowerCase.startsWith("insert")) {
                  Log.logMsg("=================================================================================")
                  Log.logMsg("executing query: \r\n" + a.replace("""{tbl_dt}""", dt.toString).replace("""{run_date}""",dt.toString).replace("flare_8","dpi_bsl"))
                  Log.logMsg("=================================================================================")
                  jDBCManager.executeQuery(a.replace("""{tbl_dt}""",dt.toString).replace("""{run_date}""",dt.toString).replace("flare_8","dpi_bsl"))
                }
...
class JDBCManager(private val url: String, private val user: String, private val pass: String) {
  ...
  def executeQuery(query: String) :String = {
    ...
    var result=1
    var status="Start"
    val table_name=query.substring(query.toLowerCase.indexOf("dpi_bsl."),query.toLowerCase.indexOf(" as"))
