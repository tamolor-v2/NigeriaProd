start transaction;
create table flare_8.fb_asset_extract_4G_dq_rpt8 as
with check_date as
(SELECT
...
from 
	...
	where 
		...
		and upper(name) like '%MTNN_ASSET_EXTRACT_4G_CELLS%')
where rnum=1	
group by 1),
total_records as ( 
select tbl_dt,
...
select 
	...
	ff.file_date,
	'Nigeria' as OpCo,
	'Asset Extract 4G' as feed_name, 
	'MTNN_ASSET_EXTRACT_4G_CELLS' as table_name, 
	...
	case 
		...
		end as MTNN_ASSET_EXTRACT_4G_CELLS,
...
left join
(select tbl_dt,tbl_month,count(*) as dq_issues , sum(incorrect_cgi) as incorrect_cgi, sum(incorrect_power_dbm) as incorrect_power_dbm
from cell_info_test 
