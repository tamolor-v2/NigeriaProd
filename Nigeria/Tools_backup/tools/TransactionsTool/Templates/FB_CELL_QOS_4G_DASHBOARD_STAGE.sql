...
create table flare_8.fb_CELL_QOS_4G_dq_rpt8 as
with check_date as
(SELECT
...
from ( 
	select tbl_dt,time,cell_4g,cgi,count(*) as duplicates
	from flare_8.CELL_QOS_4G 
	where tbl_dt between firstDayOfpreviousMonth and try_cast(date_format(CAST(current_date AS DATE),'%Y%m%d') as integer) 
	group by 1,2,3,4
	having count(*) > 1)
...
where file_date between firstDayOfpreviousMonth and try_cast(date_format(CAST(current_date AS DATE),'%Y%m%d') as integer)
    and msgtype = 'com.mtn.messages.cell_qos_4g'
group by 1),
file_received as ( 
select 
		tbl_dt, 
...
from (select try_cast(date_format( 
		...
		where tbl_dt between firstDayOfpreviousMonth and try_cast(date_format(CAST(current_date AS DATE),'%Y%m%d') as integer)
		and upper(name) like '%CELL_QOS_4G%')
group by 1),
total_records as ( 
...
select 
	...
	ff.file_date,
	'Nigeria' as OpCo,
	'Cell QOS 4G' as feed_name, 
	'CELL_QOS_4G' as table_name, 
