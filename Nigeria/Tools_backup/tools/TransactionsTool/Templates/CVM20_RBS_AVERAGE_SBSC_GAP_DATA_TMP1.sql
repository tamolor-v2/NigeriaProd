start transaction;
delete from  cvm_db.CVM20_RBS_AVERAGE_SBSC_GAP_DATA_TMP1  where tbl_dt=yyyymmddRunDate;
insert into  cvm_db.cvm20_rbs_average_sbsc_gap_data_tmp1 
(msisdn_key,weekid,average_sbsc_gap_data, tbl_dt) 
select 
msisdn_key,
'yyyymmddRunDate'    week_id , 
avg(date_diff ('day',  date_parse(cast(date_diff_days2   AS varchar(10) ),'%Y%m%d'),date_parse(cast(date_diff_days1 AS varchar(10) ),'%Y%m%d') )) average_sbsc_gap_data,
...
WHERE ( channel not IN ('EVD', 'DYA', 'VTU') AND bundle_type = 'DATA' )
