start transaction;
create table flare_8.FB_POPULATION_dq_rpt3 as
with check_date as
(SELECT
...
where file_date between firstDayOfpreviousMonth and try_cast(date_format(CAST(current_date AS DATE),'%Y%m%d') as integer)
    and msgtype = 'com.mtn.messages.population_ds'
group by 1),
file_received as ( 
select  
	tbl_dt, 
...
from 
	...
	where 
		...
		and upper(name) like '%POPULATION%')
where rnum=1	
...
select 
	...
	ff.file_date,
	'Nigeria' as OpCo,
	'Population Data' as feed_name, 
	'population_ds' as table_name, 
	...
	case when file_received>0 or total_records>0 then coalesce(missing_pop_source ,0) else null end as missing_pop_source, 
	case when coalesce(file_received,0)=0 and coalesce(total_records,0)=0 then '#DC5B57'  
		 when coalesce(missing_pop_source ,0)=0 then '#5CBE9A' 
		 when 1.000*missing_pop_source/total_records <= 0.05 then '#DD915F'
		 when 1.000*missing_pop_source/total_records >= 0.05 then '#DC5B57'  
		 else '#BE6CB4' end as missing_pop_source_RAG,
	case when file_received>0 or total_records>0 then coalesce(missing_pop_link ,0) else null end as missing_pop_link, 
	case when coalesce(file_received,0)=0 and coalesce(total_records,0)=0 then '#DC5B57'  
	 	 when coalesce(missing_pop_link ,0)=0 then '#5CBE9A' 
		 when 1.000*missing_pop_link/total_records <= 0.05 then '#DD915F'
		 when 1.000*missing_pop_link/total_records >= 0.05 then '#DC5B57'  
		 else '#BE6CB4' end as missing_pop_link_RAG ,
	case when file_received>0 or total_records>0 then coalesce(missing_pop_version ,0) else null end as missing_pop_version, 
	case when coalesce(file_received,0)=0 and coalesce(total_records,0)=0 then '#DC5B57'  
	 	 when coalesce(missing_pop_version ,0)=0 then '#5CBE9A' 
		 when 1.000*missing_pop_version/total_records <= 0.05 then '#DD915F'
		 when 1.000*missing_pop_version/total_records >= 0.05 then '#DC5B57'  
		 else '#BE6CB4' end as missing_pop_version_RAG ,
	case 
...
left join
(select tbl_dt,tbl_month,count(*) as dq_issues , sum(incorrect_country) as incorrect_country, sum(missing_pop_source) as missing_pop_source, sum(missing_pop_version) as missing_pop_version, sum(missing_pop_link) as missing_pop_link
from cell_info_test 
