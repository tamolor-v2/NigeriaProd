start transaction;
create table flare_8.fb_asset_extract_2g_dq_rpt9 as  
with check_date as
(SELECT
...
from ( 
	select tbl_dt,
		   try_cast(date_format( 
							date_trunc('month', date_parse(cast(tbl_dt  as varchar),'%Y%m%d') )
		 			,'%Y%m%d') as integer) as tbl_month, 
		 	gsm_cell_identity,count(*) as duplicates 
	from flare_8.MTNN_ASSET_EXTRACT_2G_CELLS 
	where tbl_dt between firstDayOfpreviousMonth and try_cast(date_format(CAST(current_date AS DATE),'%Y%m%d') as integer)
	group by 1,2,3
	having count(*) > 1)
...
where file_date between firstDayOfpreviousMonth and try_cast(date_format(CAST(current_date AS DATE),'%Y%m%d') as integer)
    and msgtype = 'com.mtn.messages.mtnn_asset_extract_2g_cells'
group by 1,2),
file_received as (  
select  
	tbl_dt, 
...
from 
	...
	where 
		...
		and upper(name) like '%MTNN_ASSET_EXTRACT_2G_CELLS%')
where rnum=1	
group by 1),
total_records as ( 
select tbl_dt,
...
select 
	...
	ff.file_date,
	'Nigeria' as OpCo,
	'Asset Extract 2G' as feed_name, 
	'MTNN_ASSET_EXTRACT_2G_CELLS' as table_name, 
	...
	case 
		...
		end as MTNN_ASSET_EXTRACT_2G_CELLS,
