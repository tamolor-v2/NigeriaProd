start transaction;
delete from kpi_reports.boost_connect_store_commission_plan_report where tbl_dt= cast(date_format(date_trunc('month', date_parse(CAST(yyyymmddRunDate AS varchar(10)), '%Y%m%d')) + interval '-1' day, '%Y%m%d') as int);


nsert into kpi_reports.boost_connect_store_commission_plan_report 
(BusinessUnitName,
SourceTrxNumber,
TransactionType,
SourceEventDate,
