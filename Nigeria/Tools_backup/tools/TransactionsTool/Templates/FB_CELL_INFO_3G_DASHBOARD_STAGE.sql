start transaction;
create table flare_8.fb_cell_info_3g_dq_rpt9 as
with check_date as
(SELECT
...
where file_date between firstDayOfpreviousMonth and try_cast(date_format(CAST(current_date AS DATE),'%Y%m%d') as integer)
    and msgtype = 'com.mtn.messages.cell_info_3g'
group by 1),
file_received as ( 
select  
	tbl_dt, 
...
from 
	...
	where 
		...
		and upper(name) like '%CELL_INFO_3G%')
where rnum=1	
...
select 
	...
	ff.file_date,
	'Nigeria' as OpCo,
	'Cell Info 3G' as feed_name, 
	'CELL_INFO_3G' as table_name, 
...
left join
(select tbl_dt,tbl_month,count(*) as dq_issues , sum(incorrect_cell_name) as incorrect_cell_name, sum(incorrect_cgi) as incorrect_cgi, sum(missing_frequency) as missing_frequency, sum(missing_site) as missing_site
from cell_info_test 
