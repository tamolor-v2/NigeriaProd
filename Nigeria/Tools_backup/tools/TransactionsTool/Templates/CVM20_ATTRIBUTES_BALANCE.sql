...
select 
 msisdn_key
 ,'balance_transfer_in_cnt' attribute
 ,sum(rec_count) attribute_value
from nigeria.daas_daily_usage_by_msisdn ma
where date_key between yyyymmddRunDate and yyyyRunDateWeek
and regexp_like(upper(event_type), 'RECEIVE')
group by msisdn_key
union all
select 
 msisdn_key
 ,'balance_transfer_out_amount' attribute
 ,sum(amount) attribute_value
from nigeria.daas_daily_usage_by_msisdn ma
where date_key between yyyymmddRunDate and yyyyRunDateWeek
and regexp_like(upper(event_type), 'RECEIVE')
group by msisdn_key
...
select 
 msisdn_key
 ,'balance_transfer_out_cnt' attribute
 ,sum(rec_count) attribute_value
from nigeria.daas_daily_usage_by_msisdn ma
where date_key between yyyymmddRunDate and yyyyRunDateWeek
and regexp_like(upper(event_type), 'SEND')
group by msisdn_key
union all
select 
 msisdn_key
 ,'balance_transfer_in_amount' attribute
 ,sum(amount) attribute_value
from nigeria.daas_daily_usage_by_msisdn ma
where date_key between yyyymmddRunDate and yyyyRunDateWeek
and regexp_like(upper(event_type), 'RECEIVE')
