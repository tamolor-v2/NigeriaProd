...
,sub.cgi muc
,try_cast (coalesce(sub.lat,'0.0') as double) muc_lat
,try_cast (coalesce(sub.lon,'0.0') as double) muc_lon
,sub.cell_tech muc_type
, bio_data.alternate_number
, coalesce(uc.KEEP_MY_NUMBER_FLAG,null,0) KEEP_MY_NUMBER_FLAG
...
where sub.tbl_dt=yyyymmddRunDate 
and sub.dola <= 180;
commit;
