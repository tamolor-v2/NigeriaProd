start transaction;
create table flare_8.fb_mtn_ng_3G_monthly_dq_rpt5 as 
with check_date as
(SELECT
...
where file_date between firstDayOfpreviousMonth and try_cast(date_format(CAST(current_date AS DATE),'%Y%m%d') as integer)
    and msgtype = 'com.mtn.messages.mtn_ng_3g_monthly'
group by 1,2),
file_received as (  
select  
	tbl_dt, 
...
from 
	...
	where 
		...
		and upper(name) like '%3G MONTHLY%')
where rnum=1	
group by 1),
total_records as ( 
select tbl_dt,
...
select 
	...
	ff.file_date,
	'Nigeria' as OpCo,
	'MTN NG 3G Monthly' as feed_name, 
	'MTN_NG_3G_MONTHLY' as table_name, 
