...
date_parse(CAST(yyyyRunDateWeek AS varchar(10)), '%Y%m%d')) attribute_value
from (select
msisdn_key,
concat('ds_last_',case
when service_code = 1 then 'voi'
when service_code = 2 then 'sms'
end,'_in') Attribute,
max(tbl_dt) as attribute_value
from  flare_8.msc_cdr
where tbl_dt between y90day and yyyyRunDateWeek
and  B_NUMBER like '234%'
and  call_direction in('2', '4')
group by  msisdn_key,
concat('ds_last_',case
when service_code = 1 then 'voi'
when service_code = 2 then 'sms'
