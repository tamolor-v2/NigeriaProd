    ...
    def executeTransaction(self,statement):
        ...
        try:
            ...
            for query in queries:
                ...
                checkQuery_lower = (query.replace(' ',''))
                if("STARTTRANSACTION" in checkQuery and len(checkQuery) < 17):
                    transaction = []
                if(("STARTTRANSACTION" not in checkQuery) and ("commit" not in checkQuery_lower) and len(checkQuery) > 17):
                    transaction.append(query)
                if("commit" in checkQuery_lower and len(checkQuery) < 7):
