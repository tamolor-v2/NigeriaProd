...
case class FeedPathsInfo(feedName: String, date: String, feedSrcDir: String, subFolder: String, feedHarDir: String, harFileName: String, fullPathToBeConsolidated: String, fullHarFileName: String)
...
object Driver {
  ...
  def getFileHandler(configFilePath: String): FileHandler = {
    ...
    logger.info(s"getting fileHandler connection... ")
  ...
  def main(args: Array[String]): Unit = {
    ...
    logger.info(s"getting jDBC connection... ")
    ...
    days.foreach(day => {
      ...
      filteredFeeds.foreach(f => {
        ...
        try {
          ...
          if (validationResultBefore == null || validationResultBefore.isEmpty) // todo fill status here
            throw new Exception("validation queries result cannot return null")

          summary.qResultBefore = validationResultBefore.map(m => m.map(a => a.mkString("|")).mkString("~")).mkString(";")


          val feedPathsInfo = getFeedPathsInfo(f, day)
          createHarFile(feedPathsInfo)


          summary.nFilesBefore = fileHandler.listFiles(feedPathsInfo.fullPathToBeConsolidated, 3, false).length
          logger.info(s"files in dir: ${feedPathsInfo.fullPathToBeConsolidated} before: " + summary.nFilesBefore)
          ...
          summary.nFilesAfter = fileHandler.listFiles(feedPathsInfo.fullPathToBeConsolidated, 3, false).length
          logger.info(s"files in dir: ${feedPathsInfo.fullPathToBeConsolidated} files after: " + summary.nFilesAfter)
          ...
          if (matched) {
            deleteHDFSFolderToTrash(feedPathsInfo.fullHarFileName)
            summary.status = "succeeded"
          } else {
            deleteHDFSFolderToTrash(feedPathsInfo.fullPathToBeConsolidated)
            restoreHarFile(feedPathsInfo)
            val validationResultAfterRestore = runVQuery(f.vQueries, vHandler, day)
            val isMatchingAfterRestore = compareQueryResult(validationResultBefore, validationResultAfterRestore)
            summary.qResultAfter = validationResultAfterRestore.map(m => m.map(a => a.mkString("|")).mkString("~")).mkString(";")

            if (isMatchingAfterRestore) {
              deleteHDFSFolderToTrash(feedPathsInfo.fullHarFileName)
              logger.info(s"feed:${f.name} date:$day failed to consolidate. Data was restored successfully from har file successfully")
              summary.status = "restored"
            } else {
              logger.info(s"feed:${f.name} date:$day failed to consolidate. validation numbers still don't match even after restore. har file: ${feedPathsInfo.fullHarFileName}")
              summary.status = "failed"

            }
          }
          val timeInfo = DateTimeUtil.getTimeInfo(feedSTime)
  ...
  def restoreHarFile(feedPathInfo: FeedPathsInfo): Unit = {
    val fullHarLocation = feedPathInfo.feedHarDir + FileHelper.FILE_SEPARATOR + feedPathInfo.harFileName + "/*"
    val fullOrgLocation = feedPathInfo.feedSrcDir + FileHelper.FILE_SEPARATOR
  ...
  def deleteHDFSFolderToTrash(path: String): Unit = {
    logger.info(s"deleting file : $path")
    val command: String = s"hadoop fs -rm -r $path"
    SysCommand.runSysCmd(command.split(" ").toSeq, errorSb = null)
  }
    ...
    if (before.length == after.length) { // making sure number of queries are matched
      ...
      for (i <- 0 until before.length) {
        ...
        if (bRows.length == aRows.length) {
          ...
          logger
            .warn(s"query #$i, doesn't have same number of records   bValue: ${bRows.length} aValue: ${aRows.length}")
          return false
        }
      }
    }
  ...
  def getFeedPathsInfo(feedCfg: Feed, day: String): FeedPathsInfo = {
    val stDate = DateTimeUtil.parseDataDate(day, DateTimeUtil.`yyyy-MM-dd`).format(DateTimeUtil.yyyyMMdd)

    val srcDir = if (feedCfg.srcDir != null && feedCfg.srcDir.nonEmpty) feedCfg.srcDir else Config.getGlobalLocationsCfg.srcDir
    val feedSrcDir = srcDir + FileHelper.FILE_SEPARATOR + feedCfg.name
    val srcSubFolders = "tbl_dt=" + stDate


    val harDir = if (feedCfg.harDir != null && feedCfg.srcDir.nonEmpty) feedCfg.harDir else Config.getGlobalLocationsCfg.harDir
    val feedHarDir = harDir + FileHelper.FILE_SEPARATOR + feedCfg.name
    val harName = feedCfg.name + "_" + stDate + ".har"

    val fullPathToBeConsolidated = feedSrcDir + FileHelper.FILE_SEPARATOR + srcSubFolders
    val fullHarFileName = feedHarDir + FileHelper.FILE_SEPARATOR + harName


    val feedPathsInfo = FeedPathsInfo(feedCfg.name, stDate, feedSrcDir, srcSubFolders, feedHarDir, harName, fullPathToBeConsolidated, fullHarFileName)
    logger.info(s"feedPathsInfo: feedSrcDir${feedPathsInfo.feedSrcDir},subFolder:${feedPathsInfo.subFolder},feedHarDir:${feedPathsInfo.feedHarDir},harFileName:${feedPathsInfo.harFileName}")
    feedPathsInfo
  }

  def createHarFile(feedPathsInfo: FeedPathsInfo): Unit = {
    logger.info(s"creating har file for feed:${feedPathsInfo.feedName} if not exits")
    ...
    if (!fileHandler.exists(feedPathsInfo.fullPathToBeConsolidated))
      throw new Exception(s"src dir ( ${feedPathsInfo.fullPathToBeConsolidated} )to be consolidated doesn't exist")


    if (!fileHandler.exists(feedPathsInfo.fullHarFileName)) {
      logger.info("creating new har file")
      val templatePlaceHolders: Map[String, String] = Map(
        "${harFileName}" -> feedPathsInfo.harFileName,
        "${PARENT_FOLDER}" -> feedPathsInfo.feedSrcDir,
        "${SUB_FOLDERS}" -> feedPathsInfo.subFolder,
        "${OUTPUT_FOLDER}" -> feedPathsInfo.feedHarDir
      )
      ...
      logger.info(s"using existing har file:${feedPathsInfo.fullHarFileName}")
    }
  }
  ...
  def runVQuery(vQuerysTemplates: List[String], vHandler: ValidationHandler, day: String): Array[Array[Array[String]]] = {
    ...
    try {
      ...
      vQuerysTemplates.foreach(vq => {
        ...
        val queryResult = vHandler.runQuery(query)
        if (queryResult != null && queryResult.nonEmpty)
  ...
  def createFeedWorkDir(feedCfg: Feed): String = {
    val workDirName = CmdLine.getWorkDir + FileHelper.FILE_SEPARATOR + feedCfg.name + "_" + DateTimeUtil.getCurrentDate(DateTimeUtil.yyyyMMdd) + FileHelper.FILE_SEPARATOR + System.currentTimeMillis()
...
object Config {
  ...
  private def setGlobalCfg(cfg: Map[String, Any]): Unit = {
    ...
    if (globalCfgMap.contains("har.dir")) {
      this.globalLocations.harDir = globalCfgMap("har.dir").asInstanceOf[String]
    }
  }
  ...
  private def setFeeds(cfg: Map[String, Any]): Unit = {
    ...
    feedsMap.foreach(feed => {
      ...
      if (feed.contains("src.dir")) {
        ...
        logger.debug(s"feedSrcDir:${f.srcDir}")
      }
