...
#------------------
mytime=$(date +"%Y-%m-%d_%H-%M-%S")
time java -Xmx30g -Xms30g -Dlog4j.configurationFile=/mnt/beegfs/tools/fileOps/log4j2.xml -jar /mnt/beegfs/tools/fileOps/FileOps_2.11-0.1-SNAPSHOT_hdfs.jar -in /mnt/beegfs/production/live/20|@1/|@2/processed -out /mnt/beegfs/tools/Crontab/logs/list/${currDate} -od -tf 10 -nt 32 -dp 15 -of -op list -mv /mnt/beegfs/production/archived/20|@1/|@2/ -re -rg "^.*_([0-9]{6})([0-9]{2}).*$" -opp "20|@1/|@2MSC_DaaS"  -nosim - -lbl listLive 2>&1  |tee "/mnt/beegfs/tools/Crontab/logs/list/${currDate}/list_live_$(mytime).log"
