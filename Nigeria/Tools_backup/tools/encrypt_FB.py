...
def addHeader(file):
    ...
    df.to_csv(file, header=["msisdn"], index=False)
...
def mergeFiles(inFile,outFile):
    ...
    outputFile.insert(1,"encrypted_msisdn",inputFile["msisdn"],True)
    outputFile.insert(2,"date",inputFile["optin_date"],True)
    
    outputFile.to_csv(r'tmpFile.txt',header=None,index=None,sep=',',mode='a')
...
def main() :
    ...
    with open(inFile,'r') as read_csv:
        ...
        for row in csv_reader:
            if(len(row)>1 and 'msisdn' not in row):
                encryptedNum = row[0]
    ...
    runCmd("cat tmpFile.txt > {0}".format(outFile))
    runCmd("rm {0}".format(r'tmpFile.txt'))
    runCmd("chmod 777 {0}".format(outFile))
    runCmd("hdfs dfs -put {0} /FlareData/output_8/FACEBOOK_DATA/".format(outFile))
