#!/bin/sh
exec scala -cp /home/daasuser/presto-jdbc-0.191.jar:/usr/hdp/current/hadoop-client/conf:/home/daasuser/FlareCluster/Flare/lib/system/ExtDependencyLibs_2.11-1.5.3.jar:/home/daasuser/FlareCluster/Flare/lib/system/ExtDependencyLibs2_2.11-1.5.3.jar -savecompiled "$0" "$@"
!#
...
//    parsArgs(Array("--configFile", "C:/Testing/dedup/dedup.conf","--date","20180927","--feed","MSC_CDR"))
    ...
    if (feed=="" || data.UniqueFields.trim.length ==0) Log.logError("Feed couldn't be found in config file",true) else Log.logMsg(feed+" Feed Metadata found")
...
if(tmpTableStartExists){
	...
	val filesToDelete=getCurrentHDFSFiles(feed)
...
class HdfsConfig {
  ...
  def getConfig(conf: HdfsConfig): (org.apache.hadoop.conf.Configuration, org.apache.hadoop.security.UserGroupInformation) = {
    ...
    if (conf.hostsList != null && conf.hostsList.length > 0) {
      val hosts = conf.hostsList.map(h => if (h != null) h.trim else null).filter(h => h != null && h.length > 0)
      if (hosts.length > 0)
...
org.apache.hadoop.security.UserGroupInformation.setConfiguration(hdfsConfig)
        org.apache.hadoop.security.UserGroupInformation.loginUserFromKeytab(conf.principal.trim, conf.keytab.trim)
 ...
 Log.logMsg("Started deleting hdfs files ")
    ...
    hdfsConf.hadoopConfig = List[(String, String)](
     ...
     ("dfs.client.failover.proxy.provider.ngdaas", "org.apache.hadoop.hdfs.server.namenode.ha.ConfiguredFailoverProxyProvider"))
...
def getCurrentHDFSFiles(feed:String):List[String]={
var (exitValue, stdoutStream, stderrStream) = runCommand(Seq("hadoop","fs","-ls",HadoopFolder+"/"+feed+"/tbl_dt="+date+"/*"))
//#, "|", "awk","-F","' '","'{print $8}'"))
...
if (exitValue ==0) 
	stdoutStream.split("\n").toList.map(a=>a.split("\\s+")(7)).filterNot(a=>a.contains("""/."""))
else
List("ERROR")
}
/*def deletePartitionFiles(fileList:List[String]):(Int,String,String)={
