#!/bin/sh
exec scala -cp /nas/share05/tools/presto-jdbc-0.191.jar:/usr/hdp/current/hadoop-client/conf:/home/daasuser/FlareCluster/Flare/lib/system/ExtDependencyLibs_2.11-1.5.5.jar:/home/daasuser/FlareCluster/Flare/lib/system/ExtDependencyLibs2_2.11-1.5.5.jar -savecompiled "$0" "$@"
!#
...
object DeduplicationScript {
  ...
  var HadoopFolder = ""
  val CreateTempTable =
    """create table {schema_name}.{table_name_tmp} as select *, {md5_compute} as md5_key, row_number() over( partition by {md5_compute} ) as dedup_rank from {schema_name}.{table_name} where tbl_dt= {tbl_dt} and hr='{hr}'"""
//  val CreateTempTable_digit_1 =
//  """create table {schema_name}.{table_name_tmp} as select *, {md5_compute} as md5_key, row_number() over( partition by {md5_compute} order by file_name, kamanja_loaded_date desc) as dedup_rank from {schema_name}.{table_name} where tbl_dt= {tbl_dt} and hr='{hr}' and (substr(trim(cast(msisdn_key as varchar)),-1)='{last_digit_1}'  or substr(trim(cast(msisdn_key as varchar)),-1)='{last_digit_2}' )"""
//  val CreateTempTable_digit_5 =
//  """create table {schema_name}.{table_name_tmp} as select *, {md5_compute} as md5_key, row_number() over( partition by {md5_compute} order by file_name, kamanja_loaded_date desc) as dedup_rank from {schema_name}.{table_name} where tbl_dt= {tbl_dt} and hr='{hr}' and (substr(trim(cast(msisdn_key as varchar)),-1)='{last_digit_1}' or substr(trim(cast(msisdn_key as varchar)),-1)='{last_digit_2}' or substr(trim(cast(msisdn_key as varchar)),-1)='{last_digit_3}' or substr(trim(cast(msisdn_key as varchar)),-1)='{last_digit_4}' or substr(trim(cast(msisdn_key as varchar)),-1)='{last_digit_5}')"""

  val insertTempTable =
    """insert into {schema_name}.{table_name_tmp}  select *, {md5_compute} as md5_key, row_number() over( partition by {md5_compute} ) as dedup_rank from {schema_name}.{table_name} where tbl_dt= {tbl_dt} and hr='{hr}' """
//val insertToTempTable_digit_1 =
//   """insert into {schema_name}.{table_name_tmp} ( select *, {md5_compute} as md5_key, row_number() over( partition by {md5_compute} order by file_name, kamanja_loaded_date desc) as dedup_rank from {schema_name}.{table_name} where tbl_dt= {tbl_dt} and hr='{hr}' and (substr(trim(cast(msisdn_key as varchar)),-1)='{last_digit_1}'  or substr(trim(cast(msisdn_key as varchar)),-1)='{last_digit_2}' ))"""
//val insertToTempTable_digit_5 =
//   """insert into {schema_name}.{table_name_tmp} ( select *, {md5_compute} as md5_key, row_number() over( partition by {md5_compute} order by file_name, kamanja_loaded_date desc) as dedup_rank from {schema_name}.{table_name} where tbl_dt= {tbl_dt} and hr='{hr}' and (substr(trim(cast(msisdn_key as varchar)),-1)='{last_digit_1}'  or substr(trim(cast(msisdn_key as varchar)),-1)='{last_digit_2}' or substr(trim(cast(msisdn_key as varchar)),-1)='{last_digit_3}' or substr(trim(cast(msisdn_key as varchar)),-1)='{last_digit_4}' or substr(trim(cast(msisdn_key as varchar)),-1)='{last_digit_5}'))"""

  val OriginPartitionCount =
    "select count(*) from {schema_name}.{table_name} where tbl_dt= {tbl_dt} and hr='{hr}'"
  val TempPartitionCount =
    "select count(*) from {schema_name}.{table_name_tmp} where tbl_dt= {tbl_dt} and hr='{hr}' and dedup_rank = 1"
  ...
  val CountDuplicate = "select coalesce(sum(cnt), 0) as cnt from (select {md5_compute},(count(*)-1) cnt  from {schema_name}.{table_name} where " +
    "tbl_dt={tbl_dt} and hr='{hr}' group by {md5_compute} having count(*)>1)"
  val hivecountDuplicate = "select coalesce(sum(cnt), 0) as cnt from (select {md5_compute},(count(*)-1) cnt  from {schema_name}.{table_name} where " +
    "tbl_dt={tbl_dt} and hr='{hr}' group by {md5_compute} having count(*)>1) a "
  val OriginalPartitionOverWrite =
    "insert into {schema_name}.{table_name}  select {field_List} from {schema_name}.{table_name_tmp} where tbl_dt={tbl_dt} and hr='{hr}' and dedup_rank = 1"
  val DupPartitionOverWrite =
    "insert into {schema_name}.dupdata select md5_key, cast(dedup_rank as int), file_name, file_offset, kamanja_loaded_date, file_mod_date, date_key, msisdn_key, original_timestamp_enrich, localtimestamp as deduptime, '{table_name}' as feedtype, tbl_dt from {schema_name}.{table_name_tmp} where tbl_dt={tbl_dt} and hr='{hr}' and dedup_rank > 1"
  var fieldList = ""
  val jDBCManager = new JDBCManager(
    "jdbc:presto://10.1.197.145:8999/hive5/kamanja_test",
    "daasuser",
    null)
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(feedsList)
    dates.foreach(date => {
      runForDay(date, feedsList,hr)
    })
  }


  def runForDay(date: Int, feedsList: Map[String, Feed] , hr : String): Unit = {
...
//    parsArgs(Array("--configFile", "C:/Testing/dedup/dedup.conf","--date","20180927","--feed","MSC_CDR"))
    ...
    temp_table_name = feed + "_tmp_" + date.toString+"_"+hr
    val tmpTableStartExists =
      jDBCManager.tableExists(schema_name, temp_table_name)
    if (tmpTableStartExists) {
      val tmpCountForStart = getCount(feed, TempPartitionCount, data, date,hr)
    ...
    val duplicateCount = getCount(feed, CountDuplicate, data, date,hr)
    if (duplicateCount > 0) {
      Log.logMsg("Count of duplicates = " + duplicateCount)
      val originalCount = getCount(feed, OriginPartitionCount, data, date,hr)
      Log.logMsg("Count of Original Partition = " + originalCount)
      val filesToDelete = getCurrentHDFSFiles(feed, date,hr)
...
//      {
 //     if (hr=="VOICE")
 //      createTempTablewithefivedigit(feed, CreateTempTable_digit_5, insertToTempTable_digit_5,insertTempTable, data, date,hr) 
 //      else
       createTemp =
        createTempTable(feed, CreateTempTable, insertTempTable, data, date,hr)
 //     }
 //     if (hr=="DATA")
 //       createTempTablewithonedigit(feed, CreateTempTable_digit_1, insertToTempTable_digit_1,insertTempTable, data, date,hr)

      val tmpCount = getCount(feed, TempPartitionCount, data, date,hr)
      ...
      if (originalCount == (tmpCount + duplicateCount)) {
        ...
        if (exitCode == 0) {
          ...
          populatePartition(feed, OriginalPartitionOverWrite, date,hr)
if (!feed.equalsIgnoreCase("dpi_cdr_unpack"))
          populatePartition(feed, DupPartitionOverWrite, date,hr)
      ...
      Log.logMsg("No Duplication found for " + feed + " in partition:" + date +" in SubPartion: " +  hr)
    Log.logMsg("Finished")
  }
...
//BUGBUG:: Not handled any exception here
  ...
  def populatePartition(feed: String, sqlQuery: String, date: Int, hr:String) = {
    val query = sqlQuery
  ...
  def getCurrentHDFSFiles(feed: String, date: Int ,hr:String): List[String] = {
    var (exitValue, stdoutStream, stderrStream) = if (feed.equalsIgnoreCase("dpi_cdr"))
     runCommand(
      Seq("hadoop",
          "fs",
          "-ls",
          HadoopFolder + "/" + feed + "_new/tbl_dt=" + date +"/hr="+hr +"/*"))
else if (feed.equalsIgnoreCase("dpi_cdr_unpack"))
runCommand(
      Seq("hadoop",
          "fs",
          "-ls",
          HadoopFolder + "/DpiCdrUnpack/tbl_dt=" + date +"/hr="+hr +"/*"))
else
 runCommand(
      Seq("hadoop",
          "fs",
          "-ls",
          HadoopFolder + "/" + feed + "/tbl_dt=" + date +"/hr="+hr +"/*"))
//#, "|", "awk","-F","' '","'{print $8}'"))
...
//    columns
  ...
  def createTempTablewithonedigit(feed: String,
                      sqlCreateQuery: String,
                      sqlInserttoQuery: String,
                      sqlInsertQuery: String,
                      data: Feed, date: Int ,hr: String) = {
    var query = ""
    val tmpTableExists =
    ...
    //val tmpTableExists=true
    if (tmpTableExists == false) {
      while (last_digit_1 <= 9)
      {
      if (last_digit_1==0)
      {
      last_digit_2=last_digit_1+1
      query = sqlCreateQuery
        .replace("""{schema_name}""", schema_name)
        .replace("""{table_name}""", feed)
        .replace("{table_name_tmp}", temp_table_name)
        .replace("""{tbl_dt}""", date.toString)
        .replace("""'{hr}'""",hr )
        .replace("""{last_digit_1}""",last_digit_1.toString )
        .replace("""{last_digit_2}""",last_digit_2.toString )
        .replace("""{field_List}""", fieldList)
        .replace("{md5_compute}", data.md5_compute)
            Log.logMsg("Executing Query: " + query)

        jDBCManager.executeQueryOnce(query, temp_table_name)
        last_digit_1=last_digit_1+2
       }
      else
       {
        last_digit_2=last_digit_1+1
        query = sqlInserttoQuery
        .replace("""{schema_name}""", schema_name)
        .replace("""{table_name}""", feed)
        .replace("{table_name_tmp}", temp_table_name)
        .replace("""{tbl_dt}""", date.toString)
 ...
 def createTempTablewithefivedigit(feed: String,
                      sqlCreateQuery: String,
                      sqlInserttoQuery: String,
                      sqlInsertQuery: String,
                      data: Feed, date: Int ,hr: String) = {
    ...
    if (tmpTableExists == false) {
      ...
      {
      last_digit_2=last_digit_1+1
      last_digit_3=last_digit_1+2
      last_digit_4=last_digit_1+3
      last_digit_5=last_digit_1+4
        query = sqlCreateQuery
        .replace("""{schema_name}""", schema_name)
        .replace("""{table_name}""", feed)
        .replace("{table_name_tmp}", temp_table_name)
        ...
        .replace("""{last_digit_1}""",last_digit_1.toString )
        .replace("""{last_digit_2}""",last_digit_2.toString )
	.replace("""{last_digit_3}""",last_digit_3.toString )
	.replace("""{last_digit_4}""",last_digit_4.toString )
	.replace("""{last_digit_5}""",last_digit_5.toString )
        .replace("""{field_List}""", fieldList)
        .replace("{md5_compute}", data.md5_compute)
            Log.logMsg("Executing Query: " + query)

        jDBCManager.executeQueryOnce(query, temp_table_name)
        last_digit_1=last_digit_1+5
       }
      else
       {
        last_digit_2=last_digit_1+1
  ...
  def getCount(feed: String, sqlQuery: String, data: Feed, date: Int, hr:String): Long = {
    var finalCount = 0L
    val query = sqlQuery
      .replace("""{schema_name}""", schema_name)
      .replace("""{table_name}""", feed)
  ...
  def parsArgs = (args: Array[String]) => {
    if (args.length >= 2)
      args.sliding(2, 2).toList.collect {
        ...
        case Array("--hr", argSubPartion: String) => hr = argSubPartion
