...
object DeduplicationScript {
  ...
  case class Feed(seq:Int,UniqueFields:String,hdfsLoc:String)
  var schema_name = ""
  var date=0
  var dedupFeed=""
  var temp_table_name=""
...
Ststem.exit(0)
  ...
  def moveCurrentData(feed:String,feed_meta:Feed):String={

    val newFolder=feed_meta.hdfsLoc+"_tmp_"+date+"/tbl_dt="+date+"/*"
    Log.logMsg("hadoop","fs","-mkdir",newFolder"
    //var (exitValue, stdoutStream, stderrStream) = runCommand(Seq("hadoop","fs","-mkdir",newFolder))
	Log.logMsg("hadoop","fs","-mv",feed_meta.hdfsLoc+"/"+feed+"/tbl_dt="+date+"/D*",newFolder)
var (exitValue3, stdoutStream3, stderrStream3) = runCommand(Seq("/opt/presto/bin/presto","--server","master01004:8099", "--catalog","hive5","--execute", "show create table "+feed_name))

    //var (exitValue1, stdoutStream1, stderrStream1) = runCommand(Seq("hadoop","fs","-mv",feed_meta.hdfsLoc+"/"+feed+"/tbl_dt="+date+"/D*",newFolder))
    newFolder
  }
...
def getListOfHDFSFiles()={

}

  def deletePartition(feed:String):(Int,String,String)={
    println("hadoop fs -rm "+HadoopFolder+"/"+feed+"/tbl_dt="+date+"/*")
    var (exitValue, stdoutStream, stderrStream) = runCommand(Seq("hadoop","fs","-rm",HadoopFolder+"/"+feed+"/tbl_dt="+date+"/*"))
  ...
  def processfeeds(metadata: String): Feed = {
    ...
    for (a <- result) {
      ...
      else if (kv(0).toLowerCase == "hdfs_location")
        hdfsLocation=kv(1)
    }
    return Feed( seq, unique_fields,hdfsLocation)
  }
}
