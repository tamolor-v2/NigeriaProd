...
/usr/bin/presto --server 10.1.197.145:8999 --catalog hive5 --schema nigeria --output-format TSV_HEADER -f $sqldropfile &>> $logfile
#hive -S -f $sqldropfile

#Create
cp $sqltemplatecreatefile $sqlcreatefile
sed -i "s/yyyymmdd/$rundate/g" $sqlcreatefile
/usr/bin/presto --server 10.1.197.145:8999 --catalog hive5 --schema nigeria --output-format TSV_HEADER -f $sqlcreatefile &>> $logfile 
...
/usr/bin/presto --server 10.1.197.145:8999 --catalog hive5 --schema nigeria --output-format TSV_HEADER -f $sqlspoolfile > $spoolfiletmp 
