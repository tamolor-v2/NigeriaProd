start transaction;
delete from nigeria.last_recharge_type where tbl_dt=yyyymmdd;
...
yyyymmdd as tbl_dt
from nigeria.last_recharge_type where tbl_dt= cast(date_format(date_add('day',-1,date_parse(cast(yyyymmdd as varchar),'%Y%m%d')),'%Y%m%d') as int)
)
