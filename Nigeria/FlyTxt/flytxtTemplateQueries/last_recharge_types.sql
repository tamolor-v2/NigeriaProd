start transaction;
delete from nigeria.last_recharge_types where tbl_dt=yyyymmdd;
...
on (b.tbl_dt=yyyymmdd and a.msisdn_key=b.msisdn_key and b.aggr='daily' and b.is_in_today_sdp and
a.tbl_dt=cast(date_format(date_add('day',-1,date_parse(cast(yyyymmdd as varchar),'%Y%m%d')),'%Y%m%d') as int))
union all
