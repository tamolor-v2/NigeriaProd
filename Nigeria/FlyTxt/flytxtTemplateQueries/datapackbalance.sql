start transaction;
delete from nigeria.datapackbalance where tbl_dt=yyyymmdd;
insert into nigeria.datapackbalance
select 
a.msisdn_key,
coalesce(SUM(try_cast(dedicatedaccountbalance as double)),0.0) data_balance,
SUM(CASE WHEN element_at(wallets.daid_map,try_cast(dedicatedaccountid as int)) = 'CHARGEABLE' THEN 
try_cast(dedicatedaccountbalance as double) ELSE 0.0 END) data_remaining_pack_balance,
a.tbl_dt
